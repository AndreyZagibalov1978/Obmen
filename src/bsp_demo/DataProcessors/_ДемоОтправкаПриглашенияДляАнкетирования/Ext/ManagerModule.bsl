...
Процедура СформироватьИОтправитьПисьмо(ПараметрыВызоваСервера, АдресХранилища) Экспорт
	...
	Если УчетныеЗаписи.Количество() = 0 Тогда
		Попытка
			ВызватьИсключение НСтр("ru = 'Почта не указана или содержит некорректные сведения.'");
		Исключение
			ЗаполнитьРезультат(АдресХранилища, ИнформацияОбОшибке());
		КонецПопытки;
		Возврат;
	Иначе
	...
	ТипТекста = ?(Сообщение.ДополнительныеПараметры.ФорматПисьма = Перечисления.СпособыРедактированияЭлектронныхПисем.HTML, 
		"HTML", "ПростойТекст");
...
Процедура ЗаполнитьРезультат(АдресХранилища, ИнформацияОбОшибке = Неопределено)
	
	Результат = Новый Структура("Успешно, ИнформацияОбОшибке", Истина);
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("СформироватьИОтправитьПисьмо", Истина);
