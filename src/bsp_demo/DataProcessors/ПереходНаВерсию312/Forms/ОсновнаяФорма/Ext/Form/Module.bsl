...
Функция ЗапуститьФоновоеЗадание()
	...
	Если Метаданные.Обработки.Содержит(МетаданныеОбработки) Тогда
		Задание = ДлительныеОперации.ВыполнитьВФоне("Обработка.ПереходНаВерсию312.МодульОбъекта.Внедрение",
			ПараметрыПроцедуры, ПараметрыЗадания);
	Иначе
		Задание = ДлительныеОперации.ВыполнитьВФоне("ВнешняяОбработка.ПереходНаВерсию312.МодульОбъекта.Внедрение",
			ПараметрыПроцедуры, ПараметрыЗадания);
	КонецЕсли;
...
Процедура ПриОбновленииПрогрессаФоновогоЗадания(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Статус = "Выполняется"
	   И Результат.Прогресс <> Неопределено Тогда
		...
		ФоновоеЗаданиеСостояние = Результат.Прогресс.Текст;
	КонецЕсли;
...
//
&НаКлиенте
Процедура ПослеЗавершенияФоновогоЗадания(Результат, ДополнительныеПараметры) Экспорт
	Активизировать();
	...
	Если Результат.Статус = "Выполнено" Тогда
		Если Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаОжиданиеОкончанияВнедрения Тогда
			ЗагрузитьРезультатВнедрения(Результат.АдресРезультата);
		КонецЕсли;
		ПерейтиКСтранице(+1);
	Иначе
		ПерейтиКСтранице(-1);
...
Процедура НачатьПодготовкуПараметровВыполненияВФоне(Форма)
	...
	ОбработкаОбъект = Форма.РеквизитФормыВЗначение("Объект"); // ВнешняяОбработка, ОбработкаОбъект.ПереходНаВерсию312
