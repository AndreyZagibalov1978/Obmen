...
Процедура ПодключаемыеКоманды_ПриАнализеОбъекта(Параметры, ОписаниеОбъекта)
	...
	ВводимыеНаОсновании = Параметры.ВводНаОсновании[ОписаниеОбъекта.Метаданные]; // Массив из см. ЗаписатьСообщениеПоОбъекту.СтрокаОбъекта 
	...
	Если ПроцедураМодуля = Неопределено Тогда
		...
		МетаданныеОбъекта = ОписаниеОбъекта.Метаданные; // ОбъектМетаданных - 
		...
		ИмяОбъекта = ОбщегоНазначения.ИмяБазовогоТипаПоОбъектуМетаданных(МетаданныеОбъекта)
	...
	РодительОбъекта = ОписаниеОбъекта.Родитель; // см. ЗаписатьСообщениеПоОбъекту.СтрокаОбъекта
...
Процедура ИзменитьВыгруженныеТексты(Параметры)
	...
	Номер = 0;
	Для Каждого СтрокаВида Из Параметры.ДеревоМетаданных.Строки Цикл // СтрокаДереваЗначений из см. ДеревоМетаданных
		...
		Если Не ФайлСуществует(СтрокаВида.Каталог) Тогда
			...
			Продолжить;
		КонецЕсли;
		Для Каждого СтрокаОбъекта Из СтрокаВида.Строки Цикл // СтрокаДереваЗначений из см. ДеревоМетаданных
			// Прогресс анализа.
			Номер = Номер + 1;
			...
			Для Каждого СтрокаФормы Из СтрокаОбъекта.Строки Цикл // СтрокаДереваЗначений из см. ДеревоМетаданных
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("Внедрение", Истина);
