...
// При добавлении новой переменной в модуль объекта отчета, нужно также добавить ее:
// в функцию СтруктураЗначенийПеременныхМодуляОбъектаОтчета,
// в процедуру ИнициализацияПеременныхМодуляОбъектаОтчета.
// Это необходимо для корректной работы отчета в многопоточном режиме.
...
Перем СоответствиеТиповМетаданных; // Соответствие
...
Перем ПроверятьВызовыМетодовКакПроизвольныйКод; // Булево
...
//   ВходящиеПараметры - см. ПараметрыПроверкиВнедрения
//
...
//
Функция ПроверитьВнедрение(КаталогВыгрузкиКонфигурации = "", ВходящиеПараметры = Неопределено) Экспорт
	
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		ВызватьИсключение НСтр("ru = 'Проверка внедрения в разделенном режиме не поддерживается.'");
	КонецЕсли;
	
	ПараметрыПроверки = ПараметрыПроверкиВнедрения();
	Если ВходящиеПараметры <> Неопределено Тогда
		ЗаполнитьЗначенияСвойств(ПараметрыПроверки, ВходящиеПараметры);
	КонецЕсли;
	
	ВыгрузитьКонфигурациюВXML(КаталогВыгрузкиКонфигурации);
	ВыполнитьПроверку(ПараметрыПроверки);
	
	ЗагружатьИзменения = НЕ КонфигурацияEDT И Не ПараметрыПроверки.НеЗагружатьИзменения;
	...
	Если ЗначениеЗаполнено(ПараметрыПроверки.СписокФайловДляЗагрузки) Тогда
...
// Конструктор параметров для передачи в функцию ПроверитьВнедрение.
// 
// Возвращаемое значение:
//   Структура:
...
//
Функция ПараметрыПроверкиВнедрения() Экспорт
	
	Параметры = Новый Структура;
	Параметры.Вставить("РасширениеФайлаПроверки", "");
	Параметры.Вставить("ПолныйПутьКФайлуПроверки", "");
	Параметры.Вставить("РезультатСтрокой", Ложь);
	Параметры.Вставить("ИсправлятьОшибки", Ложь);
	Параметры.Вставить("ИсправляемыеОшибки", Новый Массив);
	Параметры.Вставить("ПроверяемыеПодсистемы", Новый Массив);
	Параметры.Вставить("НеПроверяемыеПодсистемыКонфигурации", Новый Массив);
	Параметры.Вставить("НеЗагружатьИзменения", Ложь);
	Параметры.Вставить("ТаблицаИсключений", Неопределено);
	Параметры.Вставить("СписокФайловДляЗагрузки", "");
	Параметры.Вставить("ПодключеныТолькоСлужебныеРасширения", Ложь);
...
Процедура Подключаемый_БазоваяФункциональность_ПроверитьВнедрение()
	...
	Если ПроверятьВызовыМетодовКакПроизвольныйКод Тогда
		ПроверитьВызовыМетодовКакПроизвольныйКод();
	КонецЕсли;
	...
	ПроверитьКорневыеМодулиКонфигурации();
	ПроверитьИспользованиеПрокси();
...
Процедура Подключаемый_БизнесПроцессыИЗадачи_ПроверитьВнедрение()
	...
	Для Каждого Тип Из ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ВсеОбъектыАдресации, "ТипЗначения").Типы() Цикл
	...
	Для Каждого ИмяПредопределенногоЭлемента Из ПредопределенныеОбъектыАдресации Цикл
		...
		Для Каждого Тип Из ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПредопределенныйЭлемент, "ТипЗначения").Типы() Цикл
...
Процедура Подключаемый_ДатыЗапретаИзменения_ПроверитьВнедрение()
	...
	Для Каждого ИсточникДанных Из ИсточникиДанных Цикл
		ОбъектМетаданных = ОбщегоНазначения.ОбъектМетаданныхПоПолномуИмени(ИсточникДанных.Таблица);
...
Процедура Подключаемый_ЗапретРедактированияРеквизитовОбъектов_ПроверитьВнедрение()
	...
	Для Каждого Элемент Из Объекты Цикл
		МассивМетаданных.Добавить(ОбщегоНазначения.ОбъектМетаданныхПоПолномуИмени(Элемент.Ключ));
	КонецЦикла;
...
Процедура Подключаемый_ИнтерфейсOData_ПроверитьВнедрение()
	...
	Если ОшибкиРоли.Количество() > 0 Тогда
		...
		ТекстОшибкиКратко = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Некорректный состав прав роли %1.'"), Роль.Имя);
...
Процедура Подключаемый_КонтактнаяИнформация_ПроверитьВнедрение()
	...
	ПутьКДокументации = "https://its.1c.eu/db/bspdoc";
	
	ДанныеНачальногоЗаполнения = ОбновлениеИнформационнойБазыСлужебный.НаборПараметровДляЗаполненияОбъекта(МетаданныеВидовКонтактнойИнформации);
	...
	|	РодительскийВидыКонтактнойИнформации.ИмяПредопределенногоВида КАК ИмяПредопределенногоВидаРодителя,
	|	РодительскийВидыКонтактнойИнформации.ИмяПредопределенныхДанных КАК ИмяПредопределенныхДанныхРодителя
	|ИЗ
	|	Справочник.ВидыКонтактнойИнформации КАК ВидыКонтактнойИнформации
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ВидыКонтактнойИнформации КАК РодительскийВидыКонтактнойИнформации
	|		ПО ВидыКонтактнойИнформации.Родитель = РодительскийВидыКонтактнойИнформации.Ссылка
	|ГДЕ
	|	(ВидыКонтактнойИнформации.ИмяПредопределенныхДанных <> """"
	|			ИЛИ ВидыКонтактнойИнформации.ИмяПредопределенногоВида <> """")";
	
	ВидыКонтактнойИнформации = Запрос.Выполнить().Выгрузить();
	
	Позиция = ВидыКонтактнойИнформации.Количество() -1;
	Пока Позиция >=0 Цикл
		СтрокаТаблицы = ВидыКонтактнойИнформации[Позиция];
		Если СтрНачинаетсяС(СтрокаТаблицы.ИмяПредопределенныхДанных, "Удалить")
			Или СтрНачинаетсяС(СтрокаТаблицы.ИмяПредопределенногоВида, "Удалить") Тогда
			ВидыКонтактнойИнформации.Удалить(СтрокаТаблицы);
		КонецЕсли;
		Позиция = Позиция -1;
	КонецЦикла;
	
	Для каждого СтрокаТаблицы Из ДанныеНачальногоЗаполнения.ПредопределенныеДанные Цикл
		
		ИмяПредопределенного = "";
		КраткийТекстОшибки   = "";
		ПодробныйТекстОшибки = "";
		ПредопределенныйВид  = Неопределено;
		
		Если ЗначениеЗаполнено(СтрокаТаблицы.ИмяПредопределенныхДанных) Тогда
			
			Если ПроверкаНаДубли.Получить(СтрокаТаблицы.ИмяПредопределенныхДанных) = Неопределено Тогда
				
				ПредопределенныйВид = ВидыКонтактнойИнформации.Найти(СтрокаТаблицы.ИмяПредопределенныхДанных, "ИмяПредопределенныхДанных");
				Если ПредопределенныйВид = Неопределено Тогда
					
					ПодробныйТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Предопределенный вид контактной информации %1 описан в коде начального заполнения,
						...
						СтрокаТаблицы.ИмяПредопределенныхДанных, ПроверяемаяПодсистема.Синоним, ПутьКДокументации);
					КраткийТекстОшибки = НСтр("ru = 'Предопределенный вид контактной информации отсутствует в информационной базе.'");
					
				Иначе
					ИмяПредопределенного = СтрокаТаблицы.ИмяПредопределенныхДанных;
					ПроверкаНаДубли.Вставить(ИмяПредопределенного, "ИмяПредопределенныхДанных");
				КонецЕсли;
				
			Иначе
				ПодробныйТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Предопределенный вид контактной информации %1 уже описан в реквизите %2 кода начального заполнения.
					|Необходимо оставить только одно определение и удалить дублирующий код.'"), 
					СтрокаТаблицы.ИмяПредопределенныхДанных, ПроверкаНаДубли.Получить(СтрокаТаблицы.ИмяПредопределенныхДанных));
				КраткийТекстОшибки = НСтр("ru = 'Код начального заполнения содержит дублирование имен предопределенных видов контактной информации.'");
			КонецЕсли;
			
		ИначеЕсли ЗначениеЗаполнено(СтрокаТаблицы.ИмяПредопределенногоВида) Тогда
			
			Если ПроверкаНаДубли.Получить(СтрокаТаблицы.ИмяПредопределенногоВида) = Неопределено Тогда

				ПредопределенныйВид = ВидыКонтактнойИнформации.Найти(СтрокаТаблицы.ИмяПредопределенногоВида, "ИмяПредопределенногоВида");
				Если ПредопределенныйВид = Неопределено Тогда
					
					ПодробныйТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Предопределенный вид контактной информации %1 описан в процедуре %2 кода начального заполнения,
						...
						СтрокаТаблицы.ИмяПредопределенногоВида, "ПриНачальномЗаполненииЭлементов", ПроверяемаяПодсистема.Синоним, ПутьКДокументации);
					
					КраткийТекстОшибки = НСтр("ru = 'Предопределенный вид контактной информации отсутствует в информационной базе'");
					
				Иначе
					ИмяПредопределенного = СтрокаТаблицы.ИмяПредопределенногоВида;
					ПроверкаНаДубли.Вставить(ИмяПредопределенного, "ИмяПредопределенногоВида");
				КонецЕсли;
			...
			Иначе
				...
				НСтр("ru = 'Предопределенный вид контактной информации %1 уже описан в реквизите %2 кода начального заполнения.
					|Необходимо оставить только одно определение и удалить дублирующий код.'"), 
					СтрокаТаблицы.ИмяПредопределенныхДанных, ПроверкаНаДубли.Получить(СтрокаТаблицы.ИмяПредопределенногоВида));
				КраткийТекстОшибки = НСтр("ru = 'Код начального заполнения содержит дублирование имен предопределенных видов контактной информации.'");
			КонецЕсли;
		...
		Иначе
			...
			ПодробныйТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'В процедуре %1 кода начального заполнения описан предопределенный вид контактной информации %2
				...
				"ПриНачальномЗаполненииЭлементов", СтрокаТаблицы.Наименование, "ИмяПредопределенногоВида",
				ПроверяемаяПодсистема.Синоним, ПутьКДокументации);
			
			КраткийТекстОшибки = НСтр("ru = 'Код начального заполнения содержит предопределенный вид контактной информации с незаполненным именем.'");
			
		КонецЕсли;
		
		Если ЗначениеЗаполнено(КраткийТекстОшибки) Тогда
			ДобавитьОшибку(МетаданныеВидовКонтактнойИнформации, КраткийТекстОшибки, ПодробныйТекстОшибки);
		КонецЕсли;
		...
		Если Не ПредопределенныйВид.ЭтоГруппа Тогда
			ВидыКонтактнойИнформации.Удалить(ПредопределенныйВид);
			Продолжить;
		КонецЕсли;
		
		Результат = ДобавитьВидКИВТаблицуВидов(ПредопределенныйВид, ИмяПредопределенного, ТаблицаВидов);
		Если ЗначениеЗаполнено(Результат.ТекстОшибки) Тогда
			ДобавитьОшибку(МетаданныеВидовКонтактнойИнформации, Результат.ЗаголовокОшибки, Результат.ТекстОшибки);
		КонецЕсли;
	...
	Для Каждого ПредопределенныйВид Из ВидыКонтактнойИнформации Цикл
		
		Если ЗначениеЗаполнено(ПредопределенныйВид.ИмяПредопределенныхДанных)  Тогда
			
			Если Метаданные.Языки.Количество() > 1 Тогда
				
				ИмяРеквизитаНаименование = МетаданныеВидовКонтактнойИнформации.СтандартныеРеквизиты.Наименование.Имя;
				
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В конфигурации с несколькими языками стандартный реквизит %1
					...
					|Подробнее см. документацию по внедрению подсистемы ""%5"" 1С:Библиотеки стандартных подсистем: %6''"), ИмяРеквизитаНаименование,
				ПредопределенныйВид.Наименование, "ПриНачальномЗаполненииЭлементов", "ЗаполнитьМультиязычныйРеквизит",
				ПроверяемаяПодсистема.Синоним, ПутьКДокументации);
				
				КраткийТекстОшибки = НСтр("ru = 'Отсутствует заполнение наименования в коде начального заполнения.'");
				
				ДобавитьОшибку(МетаданныеВидовКонтактнойИнформации, КраткийТекстОшибки, ТекстОшибки);
			КонецЕсли;
			...
			Результат = ДобавитьВидКИВТаблицуВидов(ПредопределенныйВид, ПредопределенныйВид.ИмяПредопределенныхДанных, ТаблицаВидов);
			Если ЗначениеЗаполнено(Результат.ТекстОшибки) Тогда
				ДобавитьОшибку(МетаданныеВидовКонтактнойИнформации, Результат.ЗаголовокОшибки, Результат.ТекстОшибки);
			КонецЕсли;
		...
		Иначе
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'В информационной базе присутствует вид контактной информации %1 
			|описание которого отсутствует в процедуре %2 кода начального заполнения. Необходимо добавить код заполнения.
			|Подробнее см. документацию по внедрению подсистемы ""%3"" 1С:Библиотеки стандартных подсистем: %4'"),
			ПредопределенныйВид.ИмяПредопределенногоВида, "ПриНачальномЗаполненииЭлементов", ПроверяемаяПодсистема.Синоним, ПутьКДокументации);
			
			ДобавитьОшибку(МетаданныеВидовКонтактнойИнформации, 
				НСтр("ru = 'В код начального заполнения отсутствует описание переопределенного вида контактной информации.'"), ТекстОшибки);
	...
	МассивТипов.Добавить(СоставТипа(НСтр("ru = 'Отсутствует предопределенная группа справочника Виды контактной информации.'"), ОбъектыВладельцы));
	...
	Для Каждого ПроверяемыйОбъект Из ТаблицаВидов Цикл
		...
		Если ЗначениеЗаполнено(МодульФормы.Структура) Тогда
			ОчиститьПеременнуюМодульФормы(МодульФормы.Структура.Содержимое);
			МодульФормы = Неопределено;
		КонецЕсли;
	КонецЦикла;
...
Процедура Подключаемый_ОбновлениеВерсииИБ_ПроверитьВнедрение()
	...
	ПроверитьОтложенныеОбработчикиНаКорректностьСвойств();
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаВМоделиСервиса") Тогда
		ПроверитьОтложенныеОбработчикиНаНаличиеНеразделенныхИзменяемыхОбъектов();
	КонецЕсли;
	...
	Для Каждого ОбъектСНачальнымЗаполнением Из ОбъектыСНачальнымЗаполнением Цикл
		...
		СодержитПредопределенные =  СтрНайти(ОбъектСНачальнымЗаполнением.Значение, "Справочник") > 0 
			Или СтрНайти(ОбъектСНачальнымЗаполнением.Значение, "ПланВидовХарактеристик") > 0
			Или СтрНайти(ОбъектСНачальнымЗаполнением.Значение, "ПланСчетов") > 0 
			Или СтрНайти(ОбъектСНачальнымЗаполнением.Значение, "ПланВидовРасчета") > 0;
		
		Если Не СодержитПредопределенные Тогда
			Продолжить
		КонецЕсли;
...
Процедура Подключаемый_ПодключаемыеКоманды_ПроверитьВнедрение()
	...
	ПустыеНастройкиИсточника.Вставить("ПечатьНастройкиПечати",          Ложь);
	...
	ПустыеНастройкиИсточника.Вставить("НапоминанияПользователя",        Ложь);
	
	МассивИсключенийПредставленийОбъектовНапоминанияПользователя = Новый Массив;
	МассивИсключенийПредставленийОбъектовНапоминанияПользователя.Добавить("Справочник.ИдентификаторыОбъектовМетаданных");
	...
	МассивВидов = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(ВРег(ВидыОбъектовМетаданныхСФормами), ",",, Истина);
	Для Каждого ВидВоМножественномЧисле Из МассивВидов Цикл
		...
		Для Каждого ОбъектМетаданных Из КоллекцияОбъектовМетаданных Цикл
			...
			Если ЕстьПечать Тогда
				Если НастройкиИсточника.ПечатьНастройкиПечати Тогда
					РазмещенаПриОпределенииНастроекПечати = (НайтиМетод(ТекстМодуляМенеджера, "ПриОпределенииНастроекПечати") <> Неопределено);
					Если РазмещенаПриОпределенииНастроекПечати И НастройкиПодключенногоОбъекта.ДобавитьКомандыПечати Тогда
					...
					ИначеЕсли Не РазмещенаПриОпределенииНастроекПечати Тогда
						Кратко = НСтр("ru = 'В модуле менеджера отсутствует обязательная процедура.'");
						Подробно = НСтр("ru = 'В модуле менеджера отсутствует обязательная процедура ""%1"".'");
						Подробно = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Подробно, "ПриОпределенииНастроекПечати");
						ДобавитьОшибку(ОбъектМетаданных, Кратко, Подробно);
						Продолжить;
					ИначеЕсли Не НастройкиПодключенногоОбъекта.ДобавитьКомандыПечати Тогда
						МенеджерОбъекта = ОбщегоНазначения.МенеджерОбъектаПоПолномуИмени(ОбъектМетаданных.ПолноеИмя());
						НастройкиОбъекта = Новый Структура("ПриОпределенииПолучателей, ПриДобавленииКомандПечати", Ложь, Ложь);
						Попытка
							МенеджерОбъекта.ПриОпределенииНастроекПечати(НастройкиОбъекта);
						Исключение
						 	Кратко = НСтр("ru = 'В процедура модуля менеджера вызвала ошибку.'");
							Подробно = НСтр("ru = 'В модуле менеджера, процедура ""%1"" вызвала ошибку.'");
							Подробно = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Подробно, "ПриОпределенииНастроекПечати");
							ДобавитьОшибку(ОбъектМетаданных, Кратко, Подробно);
							Продолжить;
						КонецПопытки;
						
						Если НастройкиОбъекта.ПриОпределенииПолучателей И НайтиМетод(ТекстМодуляМенеджера, "ПриОпределенииПолучателей") = Неопределено Тогда
							ПодключаемыеКоманды_ОшибкаОтсутствияПроцедуры(ОбъектМетаданных, "ПриОпределенииПолучателей");
						ИначеЕсли Не НастройкиОбъекта.ПриОпределенииПолучателей И НайтиМетод(ТекстМодуляМенеджера, "ПриОпределенииПолучателей") <> Неопределено Тогда
							Кратко = НСтр("ru = 'В процедуре модуля менеджера отсутствует установка свойства'");
							Подробно = НСтр("ru = 'В модуле менеджера, в процедуре ""%1"" отсутствует установка свойства ""%2"".'");
							Подробно = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Подробно, "ПриОпределенииНастроекПечати", "ПриОпределенииПолучателей");
							ДобавитьОшибку(ОбъектМетаданных, Кратко, Подробно);
						КонецЕсли;
						
						Если НастройкиОбъекта.ПриДобавленииКомандПечати И НайтиМетод(ТекстМодуляМенеджера, "ДобавитьКомандыПечати") = Неопределено Тогда
							ПодключаемыеКоманды_ОшибкаОтсутствияПроцедуры(ОбъектМетаданных, "ДобавитьКомандыПечати");
						ИначеЕсли Не НастройкиОбъекта.ПриДобавленииКомандПечати И НайтиМетод(ТекстМодуляМенеджера, "ДобавитьКомандыПечати") <> Неопределено Тогда
							Кратко = НСтр("ru = 'В процедуре модуля менеджера отсутствует установка свойства'");
							Подробно = НСтр("ru = 'В модуле менеджера, в процедуре ""%1"" отсутствует установка свойства ""%2"".'");
							Подробно = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Подробно, "ПриОпределенииНастроекПечати", "ПриДобавленииКомандПечати");
							ДобавитьОшибку(ОбъектМетаданных, Кратко, Подробно);
						КонецЕсли;
					Иначе
						ПодключаемыеКоманды_ОшибкаОтсутствияПроцедуры(ОбъектМетаданных, "ПриОпределенииНастроекПечати");
					КонецЕсли;
			...
			Для Каждого Форма Из ОбъектМетаданных.Формы Цикл // ОбъектМетаданныхФорма
				...
				Если ЭтоИсточник И ФормыСНеобязательнымВнедрением.Найти(Форма) = Неопределено Тогда
					...
					Если ТипОсновногоРеквизита = Тип("ДинамическийСписок") Тогда
						...
						Если ИмяОсновнойТаблицыСпискаФормы <> Неопределено
							И ОбщегоНазначения.ОбъектМетаданныхПоПолномуИмени(ИмяОсновнойТаблицыСпискаФормы) = ОбъектМетаданных Тогда
							ТребоватьВнедрение = Истина;
						КонецЕсли;
					КонецЕсли;
				КонецЕсли;
				...
				ПереходНаНеустаревшийСпособВстраивания = Ложь;
				...
				Если Не УстаревшийСпособВстраивания Тогда
					...
					КонецЕсли;
				ИначеЕсли НайденнаяПроцедура <> Неопределено Тогда
					Отсутствующие.Добавить("ПодключаемыеКомандыКлиент.НачатьВыполнениеКоманды");
					ПереходНаНеустаревшийСпособВстраивания = Истина;
				КонецЕсли;
				
				Если УстаревшийСпособВстраивания И (Не ПереходНаНеустаревшийСпособВстраивания) Тогда
				...
				КонецЕсли;
				Если НастройкиИсточника.НапоминанияПользователя Тогда
					ПодсистемыИсточника.Добавить(
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Напоминания пользователя (см. состав определяемых типов: %1, %2)'"), "ПредметНапоминания", "ПредметНапоминанияОбъект"));
				КонецЕсли;
				...
				Если ПодсистемыИсточника.Количество() = 1 Тогда
					ОписаниеВставки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Объект подключен к подсистеме %1, требующей встраивание подсистемы ""Подключаемые команды"".'"), ОписаниеВставки);
				ИначеЕсли ПодсистемыИсточника.Количество() > 1 Тогда
					ОписаниеВставки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Объект подключен к подсистемам, требующим встраивание подсистемы ""Подключаемые команды"":
							|%1.'"), ОписаниеВставки);
				КонецЕсли;
				
				ПредставлениеОбъекта = ПредставлениеТипОбъектаМетаданных(ОбъектМетаданных);
				ЭтоИсключение = (НастройкиИсточника.НапоминанияПользователя
					И (МассивИсключенийПредставленийОбъектовНапоминанияПользователя.Найти(ПредставлениеОбъекта) <> Неопределено));
				...
				Если ТребоватьВнедрение Тогда
					Если Отсутствующие.Количество() > 0 Тогда
						...
						Если Присутствующие.Количество() > 0 Тогда
							Подробно = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru = 'В форме ""%1"" отсутствуют фрагменты кода подсистемы ""Подключаемые команды"":
									|%2.'"),
								Форма.Имя,
...
Процедура Подключаемый_РаботаВМоделиСервиса_ПроверитьВнедрение()
	...
	Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ВерсияБТС, "2.0.10.1") >= 0 Тогда

		Для каждого ОбъектМетаданных Из РезультатПроверки.НеразделенныхДанныхНетВКонтролирующейПодписке Цикл
			ДобавитьОшибку(ОбъектМетаданных, НСтр("ru = 'Контроль неразделенных данных'"),
				РезультатПроверки.ТекстИсключенияНеразделенныхДанных, Неопределено, Истина);
		КонецЦикла;
		Для каждого ОбъектМетаданных Из РезультатПроверки.ОбъектыСНесколькимиРазделителями Цикл
			ДобавитьОшибку(ОбъектМетаданных, НСтр("ru = 'Контроль неразделенных данных'"),
				РезультатПроверки.ТекстИсключенияСНесколькимиРазделителями, Неопределено, Истина);
		КонецЦикла;
	...
	Иначе
		...
		Если РезультатПроверки <> Неопределено Тогда
			Для каждого ОбъектМетаданных Из РезультатПроверки.ОбъектыМетаданных Цикл
				ДобавитьОшибку(ОбъектМетаданных, НСтр("ru = 'Контроль неразделенных данных'"),
					РезультатПроверки.ТекстИсключения, Неопределено, Истина);
			КонецЦикла;
		КонецЕсли;
	...
	Если РезультатПроверки <> Неопределено Тогда
		Для каждого ОбъектМетаданных Из РезультатПроверки.ОбъектыМетаданных Цикл
			ДобавитьОшибку(ОбъектМетаданных, НСтр("ru = 'Контроль разделенных данных'"),
				РезультатПроверки.ТекстИсключения, Неопределено, Истина);
		КонецЦикла;
	КонецЕсли;
...
Процедура Подключаемый_РассылкаОтчетов_ПроверитьВнедрение()
	...
	ГруппыКонтактнойИнформации = Новый Массив;
	ТипыПолучателейСГруппамиКонтактнойИнформации = Новый Соответствие;
	Для Каждого ТипПолучателей Из ТипыПолучателей[0].Состав Цикл
		ПолноеИмяТипаПолучателей = ТипПолучателей.ПолноеИмя();
		КонтактнаяИнформацияГруппаСсылка = КонтактнаяИнформацияГруппыТипов.Получить(СтрЗаменить(ПолноеИмяТипаПолучателей, ".", ""));
		Если КонтактнаяИнформацияГруппаСсылка = Неопределено Тогда
			...
			Продолжить;
		КонецЕсли;
		ГруппыКонтактнойИнформации.Добавить(КонтактнаяИнформацияГруппаСсылка);
		ТипыПолучателейСГруппамиКонтактнойИнформации.Вставить(ТипПолучателей, КонтактнаяИнформацияГруппаСсылка);
	КонецЦикла;
	
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	|	Родитель КАК ГруппаКонтактнойИнформации
	|ИЗ
	|	Справочник.ВидыКонтактнойИнформации
	|ГДЕ
	|	Родитель В (&ГруппыКонтактнойИнформации)
	|	И Тип = &Тип
	|СГРУППИРОВАТЬ ПО
	|	Родитель";
	Запрос.УстановитьПараметр("ГруппыКонтактнойИнформации", ГруппыКонтактнойИнформации);
	МодульУправлениеКонтактнойИнформацией = ОбщегоНазначения.ОбщийМодуль("УправлениеКонтактнойИнформацией");
	...
	Для Каждого ТипПолучателей Из ТипыПолучателейСГруппамиКонтактнойИнформации Цикл
		Выборка.Сбросить();
		ПараметрыПоиска = Новый Структура("ГруппаКонтактнойИнформации", ТипПолучателей.Значение); 
		Если НЕ Выборка.НайтиСледующий(ПараметрыПоиска) Тогда
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				Строка(ТипПолучателей.Ключ), "ПереопределитьТаблицуТиповПолучателей", "РассылкаОтчетовПереопределяемый");
			ДобавитьОшибку(ТипПолучателей.Ключ, НСтр("ru = 'Отсутствует вид контактной информации'"), ТекстОшибки);
		КонецЕсли;
	КонецЦикла;
		...
		Для Каждого ОбъектМетаданных Из ВидМетаданных Цикл
			...
			Если ТабличнаяЧастьДополнительныеРеквизиты <> Неопределено
				...
				Для Каждого ТипТабличнойЧасти Из ТипыТабличнойЧасти Цикл
					...
					Если НайденныйРеквизит = Неопределено Тогда
						ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'У объекта ""%1"" в табличной части ""%2"" отсутствует обязательный реквизит ""%3""'"), 
							ОбъектМетаданных.ПолноеИмя(), "ДополнительныеРеквизиты", ТипТабличнойЧасти.Ключ);
					...
					Иначе
						Если НайденныйРеквизит.Имя = "Значение" Тогда
							ДобавитьОшибку = Не ОбщегоНазначенияКлиентСервер.СпискиЗначенийИдентичны(
								НайденныйРеквизит.Тип.Типы(), ТипТабличнойЧасти.Значение);
						Иначе
							ДобавитьОшибку = НайденныйРеквизит.Тип <> ТипТабличнойЧасти.Значение;
						КонецЕсли;
						Если ДобавитьОшибку Тогда
							ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru = 'У объекта ""%1"" в табличной части ""%2"" тип реквизита ""%3"" не соответствует ожидаемому (%4)'"), 
								ОбъектМетаданных.ПолноеИмя(), "ДополнительныеРеквизиты", ТипТабличнойЧасти.Ключ,
								?(НайденныйРеквизит.Имя = "Значение", "Метаданные.ПланыВидовХарактеристик" + "ДополнительныеРеквизитыИСведения.Тип",
								ТипТабличнойЧасти.Значение));
							ДобавитьОшибку(ОбъектМетаданных,
								СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
	...
	Для Каждого ПредопределенныйНабор Из МодульУправлениеСвойствамиПовтИсп.ПредопределенныеНаборыСвойств() Цикл
		Если ТипЗнч(ПредопределенныйНабор.Ключ) = Тип("Строка") Тогда
			Если ИменаПредопределенных.Найти(ПредопределенныйНабор.Ключ) <> Неопределено Тогда
				...
				ОбъектМетаданных = ОбщегоНазначения.ОбъектМетаданныхПоПолномуИмени(ПерваяЧастьИмени + "." + ВтораяЧастьИмени);
...
Процедура Подключаемый_УправлениеДоступом_ПроверитьВнедрение()
	...
	Для Каждого Ошибка Из ОшибкиОграниченийДоступа Цикл
		ДобавитьОшибку(ОбщегоНазначения.ОбъектМетаданныхПоПолномуИмени(Ошибка.ПолноеИмя),
...
Процедура Подключаемый_УчетОригиналовПервичныхДокументов_ПроверитьВнедрение()
	...
	СоставОбъектовУчета = 
		СоставТипа("ОпределяемыеТипы.ОбъектСУчетомОригиналовПервичныхДокументов.Тип",,,"Справочник.ИдентификаторыОбъектовМетаданных");
		
	Для Каждого Объект Из СоставОбъектовУчета[0].Состав Цикл
		Для Каждого Форма Из Объект.Формы Цикл
			ТипОсновногоРеквизита = ТипОсновногоРеквизитаФормы(Форма);
			Если ТипОсновногоРеквизита <> Неопределено И Метаданные.НайтиПоТипу(ТипОсновногоРеквизита) = Объект
				И Не Метаданные.Обработки.Содержит(Объект) Тогда
				ТекстМодуляФормы = ТекстМодуля(Объект, Форма.Имя);
				ОбработкаОповещения = НайтиМетод(ТекстМодуляФормы, "ОбработкаОповещения");
				Если ОбработкаОповещения <> Неопределено Тогда
					ТекстМетода = НайтиВызовМетода(ОбработкаОповещения.Содержимое, "УчетОригиналовПервичныхДокументовКлиент.ОбработчикОповещенияФормаДокумента(");
					Если ТекстМетода <> Неопределено Тогда
						Если СтрНайти(ТекстМетода.Тело, "Источник") = 0 Тогда
						Кратко = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'В форме ""%1"", в процедуре %2 вставлены не все фрагменты кода'"),
							Форма.Имя, "Процедура ОбработкаОповещения");
						Подробно = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'У вызова процедуры %1 отсутствует ключевой параметр %2,
							|необходимый для корректной работы подсистемы %3.'"), "УчетОригиналовПервичныхДокументовКлиент.ОбработчикОповещенияФормаДокумента",
							"Источник", "УчетОригиналовПервичныхДокументов");
						ДобавитьОшибку(Форма, Кратко, Подробно);
						КонецЕсли;
					Иначе  
						Кратко = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'В форме ""%1"", в процедуре %2 вставлены не все фрагменты кода'"),
								Форма.Имя, "Процедура ОбработкаОповещения");
						Подробно = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Отсутствует вызов %1.'"), "УчетОригиналовПервичныхДокументовКлиент.ОбработчикОповещенияФормаДокумента");
						ДобавитьОшибку(Объект, Кратко, Подробно);
					КонецЕсли;
				Иначе
					Кратко = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'В форме ""%1"" вставлены не все фрагменты кода'"),
							Форма.Имя);
					Подробно = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Отсутствует: %1. В ней требуется наличие вставки кода %2.'"), "Процедура ОбработкаОповещения",
							"УчетОригиналовПервичныхДокументовКлиент.ОбработчикОповещенияФормаДокумента");
					ДобавитьОшибку(Объект, Кратко, Подробно);
				КонецЕсли;
			КонецЕсли;
		 КонецЦикла
	...
	Настройки = МодульУчетОригиналовПервичныхДокументов.НастройкиПодсистемы();
		
	Если Не Настройки.ВыводитьКнопкиНаФормеДокумента Тогда
	...
	Иначе
		...
		Если Не ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ПодключаемыеКоманды") Тогда
			...
			ПараметрыПроверки.ИмяПроцедурыИлиФункции = "Процедура ОбновитьКомандыСостоянияОригинала(";
			ПараметрыПроверки.СтрокаКода             = "УчетОригиналовПервичныхДокументов.ОбновитьКомандыСостоянияОригинала(";
			ПроверитьНаличиеВставкиКода(ПараметрыПроверки);

		Иначе
			ПараметрыПроверки.ИмяПроцедурыИлиФункции = "Процедура ОбновитьКомандыСостоянияОригинала";
			ПараметрыПроверки.СтрокаКода             = "ПодключаемыеКоманды.ПриСозданииНаСервере(";
			ПроверитьНаличиеВставкиКода(ПараметрыПроверки);
		КонецЕсли;
...
Процедура ДобавитьОшибку(Знач ОбъектМетаданных, КраткоеОписаниеОшибки, ПодробноеОписаниеОшибки,
	...
	ПодсистемаОбъекта    = Неопределено;
	Если ПодсистемыОбъекта <> Неопределено Тогда
		Для Каждого ПодсистемаОбъекта Из ПодсистемыОбъекта Цикл
			Если Не ПодсистемаОбъекта.Интерфейсная Тогда
				Прервать;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	ПодсистемаВерхнегоУровня = ?(ПодсистемаОбъекта = Неопределено, НСтр("ru = 'Без подсистемы'"), ПодсистемаОбъекта.Представление);
	ИмяПодсистемыВерхнегоУровня = ?(ПодсистемаОбъекта = Неопределено, "", ПодсистемаОбъекта.Имя);
	...
	Если ТипЗнч(ПодробноеОписаниеОшибки) = Тип("ИнформацияОбОшибке") Тогда
		НоваяСтрока.КраткоеОписаниеОшибки = КраткоеОписаниеОшибки + Символы.ПС + ОбработкаОшибок.КраткоеПредставлениеОшибки(ПодробноеОписаниеОшибки);
		НоваяСтрока.ПодробноеОписаниеОшибки = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ПодробноеОписаниеОшибки);
	Иначе
				...
				Если Отсутствующие.Количество() <> 0 Тогда
					...
					Если ПроверяемаяПодсистема <> Неопределено Тогда
						ДокументацияВнедрения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Подробнее см. документацию по внедрению подсистемы ""%1"" 1С:Библиотеки стандартных подсистем: https://its.1c.eu/db/bspdoc'"), 
							ПроверяемаяПодсистема.Синоним);
					Иначе
						ДокументацияВнедрения = НСтр("ru = 'Подробнее см. документацию по внедрению 1С:Библиотеки стандартных подсистем: https://its.1c.eu/db/bspdoc'");
					КонецЕсли;
...
Функция ПроверитьПраваНаРеквизитыВРоли(ДокументDOM, ИмяРоли, Исправлять)
	...
	РеквизитыВРолиПоОбъектамМетаданных = Новый Соответствие;
	ОбъектыМетаданныхСРеквизитами = Новый СписокЗначений;
	...
	Пока Истина Цикл
		...
		Иначе
			...
			ОбъектМетаданных = СоответствиеОбъектовМетаданных.Получить(ЧастиИмени[0] + "." + ЧастиИмени[1]);
			Если ОбъектМетаданных = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			ТекущаяТаблица = РеквизитыВРолиПоОбъектамМетаданных.Получить(ОбъектМетаданных);
			Если ТекущаяТаблица = Неопределено Тогда
				ТекущаяТаблица = ТаблицаРеквизитовВРоли.Скопировать();
				РеквизитыВРолиПоОбъектамМетаданных.Вставить(ОбъектМетаданных, ТекущаяТаблица);
				ОбъектыМетаданныхСРеквизитами.Добавить(ОбъектМетаданных, ОбъектМетаданных.ПолноеИмя());
			КонецЕсли;
			НовыйРеквизитВРоли = ТекущаяТаблица.Добавить();
	...
	ОбъектыМетаданныхСРеквизитами.СортироватьПоПредставлению();
	
	Для Каждого ОписаниеОбъектаМетаданных Из ОбъектыМетаданныхСРеквизитами Цикл
		ОбъектМетаданных = ОписаниеОбъектаМетаданных.Значение;
		ТекущаяТаблица = РеквизитыВРолиПоОбъектамМетаданных.Получить(ОбъектМетаданных);
		ТекущиеОшибки = Новый СписокЗначений;
		
		Для Каждого РеквизитВРоли Из ТекущаяТаблица Цикл
			...
			Если ПозицияСтандартнойТабличнойЧасти > 0 Тогда
				...
				ИмяРеквизита = Сред(ПутьДоРеквизита, 2);
			ИначеЕсли ПозицияСтандартныхРеквизитов > 0 Тогда
				...
				ИмяРеквизита = Сред(ПутьДоРеквизита, 2);
			Иначе
				ОбъектМетаданныхРеквизита = ОбщегоНазначения.ОбъектМетаданныхПоПолномуИмени(РеквизитВРоли.ПолноеИмяОбъекта);
				Если ОбъектМетаданныхРеквизита = Неопределено Тогда
					ПолноеИмяОбъекта = РеквизитВРоли.ПолноеИмяОбъекта;
				Иначе
					ПолноеИмяОбъекта = ОбъектМетаданныхРеквизита.ПолноеИмя();
				КонецЕсли;
				ЧастиТекущегоИмени = СтрРазделить(ПолноеИмяОбъекта, ".");
				ЧастиТекущегоИмени.Удалить(0);
				ЧастиТекущегоИмени.Удалить(0);
				ИмяРеквизита = СтрСоединить(ЧастиТекущегоИмени, ".");
			КонецЕсли;
			...
			Если Не ЭтоПоле Тогда // Это команда.
				Если (ПропускаемыйОбъект = Неопределено Или Не ПропускаемыйОбъект["View"])
					...
					ТекущиеОшибки.Добавить(ТекстОшибки);
			...
			Если ПраваНаПоле["View"] <> УстанавливатьПраваДляРеквизитовИТабличныхЧастейПоУмолчанию Тогда
				Если Исправлять Тогда
					УстановитьСвойство(ПравоОбъектаПросмотр, СвойствоЗначениеПрава(), 
			...
			Если ПраваНаПоле["Edit"] <> УстанавливатьПраваДляРеквизитовИТабличныхЧастейПоУмолчанию Тогда
				Если Исправлять Тогда
					УстановитьСвойство(ПравоОбъектаРедактирование, СвойствоЗначениеПрава(), 
						УстанавливатьПраваДляРеквизитовИТабличныхЧастейПоУмолчанию);
					ИмеютсяИзмененныеРоли = Истина;
				КонецЕсли;
			КонецЕсли;
	
			Если ПраваНаПоле["View"] <> УстанавливатьПраваДляРеквизитовИТабличныхЧастейПоУмолчанию 
				И ПраваНаПоле["Edit"] <> УстанавливатьПраваДляРеквизитовИТабличныхЧастейПоУмолчанию Тогда
					
				Шаблон = ?(УстанавливатьПраваДляРеквизитовИТабличныхЧастейПоУмолчанию,
					НСтр("ru = '%1 (нет прав %2 и %3)'"),
					НСтр("ru = '%1 (лишние права %2 и %3)'"));
				ТекстОшибки = ?(Исправлять, НСтр("ru = 'Исправлено.'") + " ", "")
					+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Шаблон,
						ИмяРеквизита, "Просмотр", "Редактирование");
				ТекущиеОшибки.Добавить(ТекстОшибки);
	
			ИначеЕсли ПраваНаПоле["View"] <> УстанавливатьПраваДляРеквизитовИТабличныхЧастейПоУмолчанию Тогда
	
				Шаблон = ?(УстанавливатьПраваДляРеквизитовИТабличныхЧастейПоУмолчанию,
					НСтр("ru = '%1 (нет права %2)'"),
					НСтр("ru = '%1 (лишнее право %2)'"));
				ТекстОшибки = ?(Исправлять, НСтр("ru = 'Исправлено.'") + " ", "")
					+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Шаблон, ИмяРеквизита, "Просмотр");
				ТекущиеОшибки.Добавить(ТекстОшибки);
		
			ИначеЕсли ПраваНаПоле["Edit"] <> УстанавливатьПраваДляРеквизитовИТабличныхЧастейПоУмолчанию Тогда
	
				Шаблон = ?(УстанавливатьПраваДляРеквизитовИТабличныхЧастейПоУмолчанию,
					НСтр("ru = '%1 (нет права %2)'"),
					НСтр("ru = '%1 (лишнее право %2)'"));
				ТекстОшибки = ?(Исправлять, НСтр("ru = 'Исправлено.'") + " ", "") 
					+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Шаблон, ИмяРеквизита, "Редактирование");
				ТекущиеОшибки.Добавить(ТекстОшибки);
		...
		Если Не ЗначениеЗаполнено(ТекущиеОшибки) Тогда
			Продолжить;
		КонецЕсли;
		ЗаголовокОшибки = НСтр("ru = 'Право на реквизит (табличную часть) отличается от значения по умолчанию в роли'");
		ТекущиеОшибки.СортироватьПоЗначению();
		ТекстОшибки = ОбъектМетаданных.ПолноеИмя() + ":
		|- " + СтрСоединить(ТекущиеОшибки.ВыгрузитьЗначения(), Символы.ПС + "- ") + Символы.ПС;
		ДобавитьОшибкуПравНаРеквизитыРоли(Метаданные.Роли[ИмяРоли], ЗаголовокОшибки, ТекстОшибки, , ,
			УстанавливатьПраваДляРеквизитовИТабличныхЧастейПоУмолчанию);
...
Процедура СравнитьРольСЭталоном(ИмяРоли, МакетыРоли, ПропускатьНовыеПрава, ЕстьОшибки)
	...
	ПраваПоРоли        = МакетыРоли.ПраваПоРоли.Таблица;
	ПраваПоЭталонуРоли = МакетыРоли.ПраваПоЭталонуРоли.Таблица;
	
	РезультатСравненияПрав = РезультатСравненияПравСЭталоном(ПраваПоРоли, ПраваПоЭталонуРоли);
	
	ВерсияОтчетаОтличается = ВерсияОтчетаОтличаетсяОтВерсииБиблиотеки();
	ВерсияБиблиотеки       = СтандартныеПодсистемыСервер.ВерсияБиблиотеки();
	
	ЕстьПропущенныеОшибки = Ложь;
	Для Каждого СравнениеПравОбъекта Из РезультатСравненияПрав Цикл
		
		Если Не ЭтоДемоБСП() И Не ЭтоОбъектБСП(ОбщегоНазначения.ОбъектМетаданныхПоПолномуИмени(СравнениеПравОбъекта.Ключ)) Тогда
			Продолжить;
		КонецЕсли;
		...
		Для Каждого СтрокаПрава Из СравнениеПравОбъекта.Значение Цикл
			
			Если ВерсияОтчетаОтличается И СтрокаПрава.ИспользуетсяВРоли И НЕ СтрокаПрава.ИспользуетсяВЭталоне Тогда
				ЕстьПропущенныеОшибки = Истина;
				Прервать;
			КонецЕсли;
			
			Если СтрокаПрава.ИспользуетсяВЭталоне И НЕ СтрокаПрава.ИспользуетсяВРоли И СтрокаПрава.ЗначениеЭталона = Истина Тогда
				ШаблонОшибки = ШаблонОшибкиОтсутствуетПраво;
			ИначеЕсли СтрокаПрава.ИспользуетсяВЭталоне И НЕ СтрокаПрава.ИспользуетсяВРоли И СтрокаПрава.ЗначениеЭталона = Ложь Тогда
				ШаблонОшибки = ШаблонОшибкиПрисутствуетПраво;
			ИначеЕсли НЕ СтрокаПрава.ИспользуетсяВЭталоне И СтрокаПрава.ИспользуетсяВРоли И СтрокаПрава.Значение = Истина  Тогда
				ШаблонОшибки = ШаблонОшибкиПрисутствуетПраво;
			ИначеЕсли НЕ СтрокаПрава.ИспользуетсяВЭталоне И СтрокаПрава.ИспользуетсяВРоли И СтрокаПрава.Значение = Ложь  Тогда
				ШаблонОшибки = ШаблонОшибкиОтсутствуетПраво;
			ИначеЕсли СтрокаПрава.ЗначениеЭталона <> СтрокаПрава.Значение И СтрокаПрава.ЗначениеЭталона = Истина Тогда
				ШаблонОшибки = ШаблонОшибкиОтсутствуетПраво;
			ИначеЕсли СтрокаПрава.ЗначениеЭталона <> СтрокаПрава.Значение И СтрокаПрава.ЗначениеЭталона = Ложь Тогда
				ШаблонОшибки = ШаблонОшибкиПрисутствуетПраво;
			Иначе
				Продолжить;
			КонецЕсли;
			
			Право = СинонимыВидовПрав.Получить(СтрокаПрава.Право);
			Если ПропускатьНовыеПрава И Право = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			Право = ?(Право = Неопределено, СтрокаПрава.Право, Право);
			
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонОшибки,
				СтрокаПрава.Объект, ИмяРоли, Право);
			...
			Иначе
				ЕстьОшибки = Истина;
				ОбъектЧастями = СтрРазделить(СтрокаПрава.Объект, ".");
				ПолноеИмя = ОбъектЧастями[0] + "." + ОбъектЧастями[1];
				
				ДобавитьОшибку(ОбщегоНазначения.ОбъектМетаданныхПоПолномуИмени(ПолноеИмя),
					КраткоеОписаниеОшибки,
					ТекстОшибки);
			КонецЕсли;
		КонецЦикла;
		...
		Если ФлагИзменения Тогда
			...
			ЕстьОшибки = Истина;
			ОбъектЧастями = СтрРазделить(СравнениеПравОбъекта.Ключ, ".");
			ПолноеИмя = ОбъектЧастями[0] + "." + ОбъектЧастями[1];
			ДобавитьОшибку(ОбщегоНазначения.ОбъектМетаданныхПоПолномуИмени(ПолноеИмя),
				КраткоеОписаниеОшибки,
...
Функция ПолучитьПрава(ДокументDOM, ЭтоЭталон)
	...
	Пока Истина Цикл
		...
		Если ПозицияСтандартнойТабличнойЧасти > 0 Тогда
			ОбъектМетаданных = ОбщегоНазначения.ОбъектМетаданныхПоПолномуИмени(Сред(ПолноеИмяОбъекта, 1, ПозицияСтандартнойТабличнойЧасти - 2));
		...
		ИначеЕсли ПозицияСтандартныхРеквизитов > 0 Тогда
			ОбъектМетаданных = ОбщегоНазначения.ОбъектМетаданныхПоПолномуИмени(Сред(ПолноеИмяОбъекта, 1, ПозицияСтандартныхРеквизитов - 2));
...
Функция РезультатСравненияПравСЭталоном(ПраваПоРоли, ПраваПоЭталонуРоли)
	
	ТаблицаРезультат = ПраваПоРоли.Скопировать();
	ТаблицаРезультат.Колонки.Добавить("ЗначениеЭталона");
	ТаблицаРезультат.Колонки.Добавить("ИспользуетсяВРоли");
	ТаблицаРезультат.Колонки.Добавить("ИспользуетсяВЭталоне");
	
	// Заполняем новые колонки.
	Для Каждого СтрокаТаблицы Из ТаблицаРезультат Цикл
		СтрокаТаблицы.ЗначениеЭталона      = Ложь;
		СтрокаТаблицы.ИспользуетсяВРоли    = Истина;
		СтрокаТаблицы.ИспользуетсяВЭталоне = Ложь;
	КонецЦикла;
	
	ПраваПоРоли.Индексы.Добавить("Объект, Право");
	
	// Дополняем таблицу результат строками и значениями таблицы прав по эталону роли.
	Для Каждого СтрокаТаблицыПравПоЭталонуРоли Из ПраваПоЭталонуРоли Цикл
		СтруктураПоиска = Новый Структура("Объект, Право");
		ЗаполнитьЗначенияСвойств(СтруктураПоиска, СтрокаТаблицыПравПоЭталонуРоли);
		
		СтрокиТаблицыПравПоРоли = ТаблицаРезультат.НайтиСтроки(СтруктураПоиска);
		
		Если СтрокиТаблицыПравПоРоли.Количество() = 0 Тогда
			НоваяСтрока = ТаблицаРезультат.Добавить();
			НоваяСтрока.Объект               = СтрокаТаблицыПравПоЭталонуРоли.Объект;
			НоваяСтрока.Право                = СтрокаТаблицыПравПоЭталонуРоли.Право;
			НоваяСтрока.ЗначениеЭталона      = СтрокаТаблицыПравПоЭталонуРоли.Значение;
			НоваяСтрока.ИспользуетсяВРоли    = Ложь;
			НоваяСтрока.ИспользуетсяВЭталоне = Истина;
			
			Продолжить;
		КонецЕсли;
		
		Для Каждого СтрокаТаблицыПравПоРоли Из СтрокиТаблицыПравПоРоли Цикл
			СтрокаТаблицыПравПоРоли.ЗначениеЭталона      = СтрокаТаблицыПравПоЭталонуРоли.Значение;
			СтрокаТаблицыПравПоРоли.ИспользуетсяВЭталоне = Истина;
		КонецЦикла;
	КонецЦикла;
	
	// Удалим строки, в которых права присутствуют и в конфигурации, и в эталоне, и значения прав совпадают.
	СтрокиДляУдаления = Новый Массив;
	Для Каждого СтрокаТаблицы Из ТаблицаРезультат Цикл
		Если СтрокаТаблицы.ИспользуетсяВРоли И СтрокаТаблицы.ИспользуетсяВЭталоне
				И (СтрокаТаблицы.Значение = СтрокаТаблицы.ЗначениеЭталона) Тогда
	...
	// Сформируем соответствие результат. Ключ - объект, значение - таблица с отбором по объекту.
	Для Каждого СтрокаТаблицы Из ТаблицаРезультат Цикл
		Если СоответствиеРезультат[СтрокаТаблицы.Объект] <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		ТаблицаПравОбъекта = ТаблицаРезультат.Скопировать(Новый Структура("Объект", СтрокаТаблицы.Объект));
		
		СоответствиеРезультат.Вставить(СтрокаТаблицы.Объект, ТаблицаПравОбъекта);
	КонецЦикла;
...
Функция МетодыРаботыСИнтернет()
	
	ТаблицаМетодов = Новый ТаблицаЗначений;
	ТаблицаМетодов.Колонки.Добавить("Метод");
	ТаблицаМетодов.Колонки.Добавить("ПредставлениеМетода");
	ТаблицаМетодов.Колонки.Добавить("НомерПараметраПрокси");
	
	СтрокаТаблицы = ТаблицаМетодов.Добавить();
	СтрокаТаблицы.Метод = ВРег("НовыйHTTPСоединение(");
	СтрокаТаблицы.ПредставлениеМетода = "Новый HTTPСоединение";
	СтрокаТаблицы.НомерПараметраПрокси = 5;
	
	СтрокаТаблицы = ТаблицаМетодов.Добавить();
	СтрокаТаблицы.Метод = ВРег("НовыйFTPСоединение(");
	СтрокаТаблицы.ПредставлениеМетода = "Новый FTPСоединение";
	СтрокаТаблицы.НомерПараметраПрокси = 5;
	
	СтрокаТаблицы = ТаблицаМетодов.Добавить();
	СтрокаТаблицы.Метод = ВРег("НовыйWSПрокси(");
	СтрокаТаблицы.ПредставлениеМетода = "Новый WSПрокси";
	СтрокаТаблицы.НомерПараметраПрокси = 5;
	
	СтрокаТаблицы = ТаблицаМетодов.Добавить();
	СтрокаТаблицы.Метод = ВРег("НовыйWSОпределения(");
	СтрокаТаблицы.ПредставлениеМетода = "Новый WSОпределения";
	СтрокаТаблицы.НомерПараметраПрокси = 4;
	
	СтрокаТаблицы = ТаблицаМетодов.Добавить();
	СтрокаТаблицы.Метод = ВРег("NewHTTPConnection(");
	СтрокаТаблицы.ПредставлениеМетода = "New HTTPConnection";
	СтрокаТаблицы.НомерПараметраПрокси = 5;
	
	СтрокаТаблицы = ТаблицаМетодов.Добавить();
	СтрокаТаблицы.Метод = ВРег("NewFTPConnection(");
	СтрокаТаблицы.ПредставлениеМетода = "New FTPConnection";
	СтрокаТаблицы.НомерПараметраПрокси = 5;
	
	СтрокаТаблицы = ТаблицаМетодов.Добавить();
	СтрокаТаблицы.Метод = ВРег("NewWSProxy(");
	СтрокаТаблицы.ПредставлениеМетода = "New WSProxy";
	СтрокаТаблицы.НомерПараметраПрокси = 5;
	
	СтрокаТаблицы = ТаблицаМетодов.Добавить();
	СтрокаТаблицы.Метод = ВРег("NewWSDefinitions(");
	СтрокаТаблицы.ПредставлениеМетода = "New WSDefinitions";
	СтрокаТаблицы.НомерПараметраПрокси = 4;
	
	ТаблицаМетодов.Индексы.Добавить("Метод");
	Возврат ТаблицаМетодов;
	
КонецФункции

Функция ПараметрыОшибкиИспользованияПрокси()
	
	Результат = Новый Структура;
	
	Результат.Вставить("ПолноеИмяФайла");
	Результат.Вставить("ОбъектМетаданныхПроверкиПрокси");
	Результат.Вставить("ТекстовыйДокумент");
	Результат.Вставить("НомерСтрокиВызова");
	Результат.Вставить("КоличествоСтрокВызова");
	Результат.Вставить("ПредставлениеМетода");
	
	Возврат Результат;
	
КонецФункции

Процедура ПроверитьИспользованиеПрокси()
	
	ПараметрыОшибкиИспользованияПрокси = ПараметрыОшибкиИспользованияПрокси();
	
	// Исключаем из проверки общие модули "ПолучениеФайловИзИнтернета..." и этот отчет.
	ИсключенияПроверки = Новый Массив;
	ИсключенияПроверки.Добавить("ПолучениеФайловИзИнтернета");
	ИсключенияПроверки.Добавить("GetFilesFromInternet");
	ИсключенияПроверки.Добавить("ПроверкаВнедренияБСП");
	ИсключенияПроверки.Добавить("SSLImplementationCheck");
	
	ФайлыМодулей = НайтиФайлы(КаталогВыгрузки, "*.bsl", Истина);
	Для Каждого ФайлМодуля Из ФайлыМодулей Цикл
		
		ПолноеИмяФайла = ФайлМодуля.ПолноеИмя;
		
		СвойстваОбъектаМетаданных = СвойстваОбъектаПоИмениФайла(ПолноеИмяФайла);
		ОбъектМетаданныхПроверкиПрокси = СвойстваОбъектаМетаданных.ОбъектМетаданных;
		Если ОбъектМетаданныхПроверкиПрокси = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		ИмяОбъектаМетаданныхПроверки = ОбъектМетаданныхПроверкиПрокси.Имя;
		ЭтоИсключениеПроверки = СтрНачинаетсяСПоМассиву(ИмяОбъектаМетаданныхПроверки, ИсключенияПроверки);
		Если ЭтоИсключениеПроверки.Успешно Тогда
			Продолжить;
		КонецЕсли;
		
		ПараметрыОшибкиИспользованияПрокси.ОбъектМетаданныхПроверкиПрокси = ОбъектМетаданныхПроверкиПрокси;
		ПараметрыОшибкиИспользованияПрокси.ПолноеИмяФайла = ПолноеИмяФайла;
		
		ПроверитьИспользованиеПроксиВФайле(ПараметрыОшибкиИспользованияПрокси);
		
	КонецЦикла;
	
КонецПроцедуры

Процедура ПроверитьИспользованиеПроксиВФайле(ПараметрыОшибкиИспользованияПрокси)
	
	ТекстовыйДокумент = Новый ТекстовыйДокумент;
	ТекстовыйДокумент.Прочитать(ПараметрыОшибкиИспользованияПрокси.ПолноеИмяФайла);
	ТекстМодуляИсходный = ТекстовыйДокумент.ПолучитьТекст();
	
	Если ПустаяСтрока(ТекстМодуляИсходный) Тогда
		Возврат;
	КонецЕсли;
	
	ТаблицаМетодовСПараметромПрокси = МетодыРаботыСИнтернет();
	МассивМетодовСПараметромПрокси = ТаблицаМетодовСПараметромПрокси.ВыгрузитьКолонку("Метод");
	
	МассивСловоФайл = Новый Массив;
	МассивСловоФайл.Добавить(ВРег("файл"));
	МассивСловоФайл.Добавить(ВРег("file"));
	
	МассивКонструкторWSОпределения = Новый Массив;
	МассивКонструкторWSОпределения.Добавить(ВРег("НовыйWSОпределения("));
	МассивКонструкторWSОпределения.Добавить(ВРег("NewWSDefinitions("));
	
	ПараметрыОшибкиИспользованияПрокси.ТекстовыйДокумент = ТекстовыйДокумент;
	
	ТекстМодуля = СтрЗаменить(ТекстМодуляИсходный, " ", "");
	...
	Пока Истина Цикл
		...
		ВыражениеДляПоиска = СтрСоединить(МассивМетодовСПараметромПрокси, "|");
		ВыражениеДляПоиска = СтрЗаменить(ВыражениеДляПоиска, "(", "\(");
		
		РезультатПоиска = СтрНайтиПоРегулярномуВыражению(ТекстМодуля, ВыражениеДляПоиска, 
			НаправлениеПоиска.СНачала, ПозицияМетода + 1);
		Если РезультатПоиска = Неопределено Тогда
			Прервать;
		КонецЕсли;
		
		ПозицияМетода = РезультатПоиска.НачальнаяПозиция;
		Если ПозицияМетода = 0 Тогда
			Прервать;
		КонецЕсли;
		
		НайденныйМетод = РезультатПоиска.Значение;
		
		СтрокаТаблицыМетодов = ТаблицаМетодовСПараметромПрокси.Найти(НайденныйМетод, "Метод");
		Если СтрокаТаблицыМетодов = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		ПараметрыМетода = ПараметрыМетода(ТекстМодуля, НайденныйМетод, ПозицияМетода);
		Если НЕ ПараметрыМетода.МетодНайден Тогда
			Продолжить;
		КонецЕсли;
		
		ТекстМодуляДоМетода = Лев(ТекстМодуля, ПозицияМетода - 1);
		ПоследняяСтрокаТекстМодуляДоМетода = ПоследняяСтрокаМодуля(ТекстМодуляДоМетода);
		ПозицияКомментария = СтрНайти(ПоследняяСтрокаТекстМодуляДоМетода, "//");
		
		// Если найденный конструктор находится в комментарии, то пропускаем.
		Если ПозицияКомментария > 0 Тогда
			Продолжить;
		КонецЕсли;
		
		МассивПараметровМетода = ПараметрыМетода.МассивПараметров;
		
		// Проверяем, что для конструктора "Новый WSОпределения" в параметре "МестоположениеWSDL" передано имя файла.
		Если МассивКонструкторWSОпределения.Найти(НайденныйМетод) <> Неопределено Тогда
			Если МассивПараметровМетода.Количество() > 0 Тогда
				ПервыйПараметрМетода = МассивПараметровМетода[0];
				Если СтрНайтиПоМассиву(ПервыйПараметрМетода, МассивСловоФайл).Успешно Тогда
					Продолжить;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		
		ПараметрыОшибкиИспользованияПрокси.ПредставлениеМетода   = СтрокаТаблицыМетодов.ПредставлениеМетода;
		ПараметрыОшибкиИспользованияПрокси.НомерСтрокиВызова     = ПараметрыМетода.НомерСтроки;
		ПараметрыОшибкиИспользованияПрокси.КоличествоСтрокВызова = СтрЧислоСтрок(ПараметрыМетода.ТекстВызова);
		
		НомерПараметраПрокси = СтрокаТаблицыМетодов.НомерПараметраПрокси;
		
		// Если количество параметров меньше номера параметра "Прокси", то регистрируем ошибку.
		Если МассивПараметровМетода.Количество() < НомерПараметраПрокси Тогда
			ЗарегистрироватьОшибкуИспользованияПрокси(ПараметрыОшибкиИспользованияПрокси);
			Продолжить;
		КонецЕсли;
		
		ПараметрПрокси = МассивПараметровМетода[НомерПараметраПрокси - 1];
		
		// Если параметр "Прокси" не заполнен, то регистрируем ошибку.
		Если ПустаяСтрока(ПараметрПрокси) Тогда
			ЗарегистрироватьОшибкуИспользованияПрокси(ПараметрыОшибкиИспользованияПрокси);
		КонецЕсли;
...
Процедура ЗарегистрироватьОшибкуИспользованияПрокси(ПараметрыОшибкиИспользованияПрокси)
	
	ТекстовыйДокумент = ПараметрыОшибкиИспользованияПрокси.ТекстовыйДокумент;
	
	// В текст ошибки выводим 5 строк проблемного фрагмента кода.
	НомерПервойСтроки = ПараметрыОшибкиИспользованияПрокси.НомерСтрокиВызова;
	КоличествоСтрокМетода = ПараметрыОшибкиИспользованияПрокси.КоличествоСтрокВызова;
	НомерПоследнейСтроки = Мин(НомерПервойСтроки + КоличествоСтрокМетода - 1, НомерПервойСтроки + 5);
	
	ПолнаяСтрокаМетода = "";
	Для НомерСтроки = НомерПервойСтроки По НомерПоследнейСтроки Цикл
		СтрокаМодуля = ТекстовыйДокумент.ПолучитьСтроку(НомерСтроки);
		ПолнаяСтрокаМетода = ПолнаяСтрокаМетода + СтрокаМодуля + Символы.ПС;
	КонецЦикла;
	
	ШаблонПодробногоОписанияОшибки = НСтр(
	...
	ПредставлениеМетода = ПараметрыОшибкиИспользованияПрокси.ПредставлениеМетода;
	
	ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонПодробногоОписанияОшибки,
		ПредставлениеМетода, "Прокси", "ПолучениеФайловИзИнтернета.ПолучитьПрокси", СокрЛП(ПолнаяСтрокаМетода),
		Формат(НомерПервойСтроки, "ЧГ="));
	
	ДобавитьОшибку(ПараметрыОшибкиИспользованияПрокси.ОбъектМетаданныхПроверкиПрокси,
		НСтр("ru = 'Не указан системный прокси'"),
		ТекстОшибки);
	
КонецПроцедуры

Функция УсловияПоискаМетодовВыполнить()
	
	УсловияПоиска = Новый Массив;
	
	УсловияПоиска.Добавить(НовоеУсловиеПоиска(
		"ОбщегоНазначения", "ВыполнитьМетодКонфигурации", 1, Истина, Ложь));
	
	УсловияПоиска.Добавить(НовоеУсловиеПоиска(
		"ОбщегоНазначения", "ВыполнитьМетодОбъекта", 2, Истина, Ложь));
	
	УсловияПоиска.Добавить(НовоеУсловиеПоиска(
		"ДлительныеОперации", "ВыполнитьФункцию", 2, Истина, Истина));
	
	УсловияПоиска.Добавить(НовоеУсловиеПоиска(
		"ДлительныеОперации", "ВыполнитьПроцедуру", 2, Истина, Истина));
	
	УсловияПоиска.Добавить(НовоеУсловиеПоиска(
		"ДлительныеОперации", "ВыполнитьФункциюВНесколькоПотоков", 1, Истина, Истина));
	
	УсловияПоиска.Добавить(НовоеУсловиеПоиска(
		"ДлительныеОперации", "ВыполнитьПроцедуруВНесколькоПотоков", 1, Истина, Истина));
	
	УсловияПоиска.Добавить(НовоеУсловиеПоиска(
		"ДлительныеОперации", "ВыполнитьВФоне", 1, Истина, Истина));
	
	УсловияПоиска.Добавить(НовоеУсловиеПоиска(
		"ФоновыеЗадания", "Выполнить", 1, Неопределено, Истина,
		"BackgroundJobs", "Execute"));
	
	УсловияПоиска.Добавить(НовоеУсловиеПоиска(
		"РасширенияКонфигурации", "ВыполнитьФоновоеЗаданиеСРасширениямиБазыДанных", 1, Неопределено, Истина,
		"ConfigurationExtensions", "ExecuteBackgroundJobWithDatabaseExtensions"));
	
	УсловияПоиска.Добавить(НовоеУсловиеПоиска(
		"РасширенияКонфигурации", "ВыполнитьФоновоеЗаданиеБезРасширений", 1, Неопределено, Истина,
...
Функция НовоеУсловиеПоиска(ИмяМодуля, ИмяМетода, НомерПараметра, ЭтоФункция, ВФоне, ИмяМодуля2 = "", ИмяМетода2 = "")
	
	Условие = Новый Структура;
	Условие.Вставить("ИмяМодуля", ИмяМодуля);
	Условие.Вставить("ИмяМодуля2", ?(ИмяМодуля2 = "", "Модуль" + ИмяМодуля, ИмяМодуля2));
	Условие.Вставить("ИмяМетода", ИмяМетода);
	Условие.Вставить("ИмяМетода2", ?(ИмяМетода2 = "", ИмяМетода, ИмяМетода2));
	Условие.Вставить("НомерПараметра", НомерПараметра);
	Условие.Вставить("ЭтоФункция", ЭтоФункция);
	Условие.Вставить("ВФоне", ВФоне);
	
	Возврат Условие;
	
КонецФункции

Процедура ПроверитьВызовыМетодовКакПроизвольныйКод()
	
	НастройкиПроверки = СтандартныеПодсистемыСервер.НастройкиПроверкиМетодовВызываемыхКакПроизвольныйКод();
	
	ИсключенияПроверки = Новый Соответствие;
	Для Каждого ИсключениеПроверки Из НастройкиПроверки.ИсключенияПроверки Цикл
		ПолноеИмяМодуля = ИсключениеПроверки.ПолноеИмяОбъекта;
		ИсключенияМодуля = ИсключенияПроверки.Получить(ПолноеИмяМодуля);
		Если ИсключенияМодуля = Неопределено Тогда
			ИсключенияМодуля = Новый Соответствие;
			ИсключенияПроверки.Вставить(ПолноеИмяМодуля, ИсключенияМодуля);
		КонецЕсли;
		ИменаПроцедур = ИсключенияМодуля.Получить(ИсключениеПроверки.ФрагментСодержания);
		Если ИменаПроцедур = Неопределено Тогда
			ИменаПроцедур = Новый Соответствие;
			ИсключенияМодуля.Вставить(ИсключениеПроверки.ФрагментСодержания, ИменаПроцедур);
		КонецЕсли;
		ИменаПроцедур.Вставить(ИсключениеПроверки.ИмяПроцедуры, Истина);
	КонецЦикла;
	
	УсловияПоиска = УсловияПоискаМетодовВыполнить();
	
	ДополнительныеПараметры = Новый Структура;
	ДополнительныеПараметры.Вставить("УсловияПоиска", УсловияПоиска);
	ДополнительныеПараметры.Вставить("ОбъектМетаданных", Неопределено);
	ДополнительныеПараметры.Вставить("ПолноеИмяМодуля", "");
	ДополнительныеПараметры.Вставить("Исключения", Неопределено);
	ДополнительныеПараметры.Вставить("ПроцедурыМодуля", Новый Соответствие);
	...
	Для Каждого ФайлМодуля Из ФайлыМодулей Цикл
		...
		СвойстваОбъектаМетаданных = СвойстваОбъектаПоИмениФайла(ПолноеИмяФайла);
		ОбъектМетаданных = СвойстваОбъектаМетаданных.ОбъектМетаданных;
		Если ОбъектМетаданных = Неопределено
		 Или Метаданные.ОбщиеМодули.Содержит(ОбъектМетаданных)
		   И Не ОбъектМетаданных.Сервер
		   И Не ОбъектМетаданных.ВнешнееСоединение
		   И Не ОбъектМетаданных.КлиентОбычноеПриложение Тогда
			Продолжить;
		КонецЕсли;
		...
		ИсключенияДляМодуля = ИсключенияПроверки.Получить(ОбъектМетаданных.ПолноеИмя()
			+ ?(ВРег(ФайлМодуля.Имя) = ВРег("ObjectModule"), ".МодульОбъекта", ""));
		
		ДополнительныеПараметры.ОбъектМетаданных = ОбъектМетаданных;
		ДополнительныеПараметры.ПолноеИмяМодуля = СвойстваОбъектаМетаданных.Представление;
		ДополнительныеПараметры.Исключения = ИсключенияДляМодуля;
		Если ИсключенияДляМодуля <> Неопределено Тогда
			ДополнительныеПараметры.ПроцедурыМодуля = ПроцедурыМодуля(ТекстМодуля);
		КонецЕсли;
		
		ПроверитьВызовыМетодовКакПроизвольныйКодВМодуле(ТекстМодуля, ДополнительныеПараметры);
		ПроверитьИменаУказанныхМетодовМодуля(ТекстМодуля, ФайлМодуля.Имя, ДополнительныеПараметры);
		
	КонецЦикла;
	
	Для Каждого Описание Из НастройкиПроверки.ОписаниеОбработчиков Цикл
		ПроверитьВызовыМетодовОбработчиковПодсистем(Описание.ИменаПроцедур, Описание.ЗаголовокОшибки);
	КонецЦикла;
...
//   * Ключ - Строка - имя процедуры или функции.
//   * Значение - Структура:
//      ** Начало - Число - позиция начала объявления процедуры или функции
//      ** Конец  - Число - позиция конца объявления процедуры или функции
//
Функция ПроцедурыМодуля(ТекстМодуля)
	
	// Поиск всех процедур и функций модуля без выделения параметров.
	// АПК:216-выкл - регулярное выражение для поиска.
	ВыражениеПоиска = "(?:^|\s+)(?:Функция|Процедура)\s*[a-zA-Zа-яА-Я0-9_]*\s*\(\s*(?:[^'""()]|'[^']*'|""[^""]*"")*\s*\)?(?:\r?\n|\r|.)*?(?:КонецФункции|КонецПроцедуры)";
	// АПК:216-вкл
	РезультатыПоиска = СтрНайтиВсеПоРегулярномуВыражению(ТекстМодуля, ВыражениеПоиска, Истина, Ложь);
	
	ПроцедурыМодуля = Новый Соответствие;
	Для Каждого РезультатПоиска Из РезультатыПоиска Цикл
		Содержание = СокрЛП(РезультатПоиска.Значение);
		Если СтрНачинаетсяС(ВРег(Содержание), "ФУНКЦИЯ") Тогда
			ИмяМетода = Сред(Содержание, СтрДлина("Функция") + 1);
		ИначеЕсли СтрНачинаетсяС(ВРег(Содержание), "ПРОЦЕДУРА")Тогда
			ИмяМетода = Сред(Содержание, СтрДлина("Процедура") + 1);
		Иначе
			Продолжить;
		КонецЕсли;
		...
		Если ПозицияСкобки = 0 Тогда
			Продолжить;
		КонецЕсли;
		ИмяМетода = СокрЛП(Сред(ИмяМетода, 1, ПозицияСкобки - 1));
		ПроцедурыМодуля.Вставить(ВРег(ИмяМетода), Новый Структура("Начало, Конец",
			РезультатПоиска.НачальнаяПозиция, РезультатПоиска.НачальнаяПозиция + РезультатПоиска.Длина));
	КонецЦикла;
	
	Возврат ПроцедурыМодуля;
	
КонецФункции

Процедура ПроверитьВызовыМетодовКакПроизвольныйКодВМодуле(ТекстМодуля, ДополнительныеПараметры)
	
	// Поиск всех строк с указанными элементами поиска (вызовы методов платформы, функций/процедур и их объявления).
	// Строка параметров выделяется в группу.
	Шаблон = "(?:^|\s+)(?:%1)\s*\(\s*((?:[^'""()]|'[^']*'|""[^""]*""|\([^\)]*\))*)\s*\)";
	// Пример: (?:^|\s+)(?:МодульДлительныеОперации\s*\.\s*ВыполнитьВФоне|ДлительныеОперации\s*\.\s*ВыполнитьВФоне|Функция\s*ВыполнитьВФоне|\s*ВыполнитьВФоне)\s*\(\s*((?:[^'"()]|'[^']*'|"[^"]*"|\([^\)]*\))*)\s*\)
	
	ЭлементыПоиска = Новый Массив;
	Для Каждого УсловиеПоиска Из ДополнительныеПараметры.УсловияПоиска Цикл
		Если УсловиеПоиска.ЭтоФункция = Неопределено Тогда
			// Поиск вызов методов платформы.
			ЭлементыПоиска.Добавить(УсловиеПоиска.ИмяМодуля + "\s*\.\s*" + УсловиеПоиска.ИмяМетода);
			ЭлементыПоиска.Добавить(УсловиеПоиска.ИмяМодуля2 + "\s*\.\s*" + УсловиеПоиска.ИмяМетода2);
		Иначе
			// Поиск вызовов процедуры или функции.
			ЭлементыПоиска.Добавить(УсловиеПоиска.ИмяМодуля2 + "\s*\.\s*" + УсловиеПоиска.ИмяМетода2);
			ЭлементыПоиска.Добавить(УсловиеПоиска.ИмяМодуля + "\s*\.\s*" + УсловиеПоиска.ИмяМетода);
			// Поиск объявления процедуры или функции и их вызовов в модуле объявления.
			ЭтоМодульОбъявленияМетода = ДополнительныеПараметры.ОбъектМетаданных.ПолноеИмя()
				= "ОбщийМодуль." + УсловиеПоиска.ИмяМодуля;
			Если ЭтоМодульОбъявленияМетода Тогда
				ИмяОбъявленияМетода = ?(УсловиеПоиска.ЭтоФункция, "Функция", "Процедура");
				ЭлементыПоиска.Добавить(ИмяОбъявленияМетода + "\s*" + УсловиеПоиска.ИмяМетода);
				ЭлементыПоиска.Добавить("\s*" + УсловиеПоиска.ИмяМетода);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	ВыражениеПоиска = СтрШаблон(Шаблон, СтрСоединить(ЭлементыПоиска, "|"));
	
	РезультатыПоиска = СтрНайтиВсеПоРегулярномуВыражению(ТекстМодуля, ВыражениеПоиска, Истина, Ложь);
	Для Каждого РезультатПоиска Из РезультатыПоиска Цикл
		Содержание = СокрЛП(РезультатПоиска.Значение);
		// Объявление процедуры или функции пропускается.
		Если СтрНачинаетсяС(ВРег(Содержание), "ФУНКЦИЯ")
		 Или СтрНачинаетсяС(ВРег(Содержание), "ПРОЦЕДУРА")Тогда
			Продолжить;
		КонецЕсли;
		ГруппыРезультата = РезультатПоиска.ПолучитьГруппы();
		Если ГруппыРезультата.Количество() = 0 Тогда
			Продолжить;
		КонецЕсли;
		СдвигСодержания = СтрНайти(РезультатПоиска.Значение, Содержание) - 1;
		ПозицияНачалаСтроки = СтрНайти(ТекстМодуля,
			Символы.ПС, НаправлениеПоиска.СКонца, РезультатПоиска.НачальнаяПозиция) + 1;
		ПозицияВСтроке = РезультатПоиска.НачальнаяПозиция + СдвигСодержания - ПозицияНачалаСтроки + 1;
		НачалоСтроки = Сред(ТекстМодуля, ПозицияНачалаСтроки, ПозицияВСтроке - 1);
		Если СтрНачинаетсяС(СокрЛ(НачалоСтроки), "//") Тогда
			Продолжить;
		ИначеЕсли ДополнительныеПараметры.Исключения <> Неопределено Тогда
			ИсключениеНайдено = Ложь;
			ИменаПроцедур = ДополнительныеПараметры.Исключения.Получить(СокрЛ(НачалоСтроки) + Содержание);
			Если ИменаПроцедур <> Неопределено Тогда
				Для Каждого ИмяПроцедуры Из ИменаПроцедур Цикл
					Размещение = ДополнительныеПараметры.ПроцедурыМодуля.Получить(ВРег(ИмяПроцедуры.Ключ));
					Если Размещение = Неопределено Тогда
						Продолжить;
					КонецЕсли;
					Если Размещение.Начало < РезультатПоиска.НачальнаяПозиция
					   И Размещение.Конец > РезультатПоиска.НачальнаяПозиция Тогда
			...
			Если ИсключениеНайдено Тогда
				Продолжить;
			КонецЕсли;
			Если СтрНайти(НачалоСтроки, ".ФрагментСодержания") > 0 Тогда
				Размещение = ДополнительныеПараметры.ПроцедурыМодуля.Получить(
					ВРег("ПриНастройкеПроверкиМетодовВызываемыхКакПроизвольныйКод"));
				Если Размещение <> Неопределено
				   И Размещение.Начало < РезультатПоиска.НачальнаяПозиция
				   И Размещение.Конец > РезультатПоиска.НачальнаяПозиция Тогда
					Продолжить;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		ПозицияСкобки = СтрНайти(Содержание, "(");
		СтрокаВызова = Сред(Содержание, 1, ПозицияСкобки - 1);
		ЧастиСтрокиВызова = СтрРазделить(СтрокаВызова, ".");
		ТекущееИмяМетода = ВРег(ЧастиСтрокиВызова[ЧастиСтрокиВызова.ВГраница()]);
		ТекущееУсловиеПоиска = Неопределено;
		Для Каждого УсловиеПоиска Из ДополнительныеПараметры.УсловияПоиска Цикл
			Если ВРег(УсловиеПоиска.ИмяМетода) = ТекущееИмяМетода
			 Или ВРег(УсловиеПоиска.ИмяМетода2) = ТекущееИмяМетода Тогда
				ТекущееУсловиеПоиска = УсловиеПоиска;
				Прервать;
			КонецЕсли;
		КонецЦикла;
		Если ТекущееУсловиеПоиска = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		ПараметрыИзСтроки = ПараметрыИзСтроки(ГруппыРезультата[0].Значение);
		Если ПараметрыИзСтроки.Количество() < ТекущееУсловиеПоиска.НомерПараметра Тогда
			Продолжить;
		КонецЕсли;
		Параметр = СокрЛП(ПараметрыИзСтроки[ТекущееУсловиеПоиска.НомерПараметра - 1]);
		ТекстОшибки = Неопределено;
		Если Не (СтрНачинаетсяС(Параметр, """") И СтрЗаканчиваетсяНа(Параметр, """")) Тогда
			Если СтрНачинаетсяС(ВРег(Параметр), ВРег("Метаданные.РегламентныеЗадания"))
			   И СтрЗаканчиваетсяНа(ВРег(Параметр), ВРег(".ИмяМетода")) Тогда
				Продолжить;
			КонецЕсли;
			ТекстОшибки = НСтр("ru = 'Имя вызываемой процедуры не является строковым литералом и методом регламентного задания в формате ""Метаданные.РегламентныеЗадания<.ИмяРегламентногоЗадания>.ИмяМетода"".'");
		ИначеЕсли ТекущееУсловиеПоиска.ИмяМетода = "ВыполнитьМетодОбъекта" Тогда
			Продолжить;
		Иначе
			ИмяМетодаВПараметре = Сред(Параметр, 2, СтрДлина(Параметр) - 2);
			ЧастиИмени = СтрРазделить(ИмяМетодаВПараметре, ".", Истина);
			Объект = Неопределено;
			Если ЧастиИмени.Количество() = 4 И ВРег(ЧастиИмени[2]) = "МОДУЛЬОБЪЕКТА" Тогда
				ИмяМетодаВПараметре = ЧастиИмени[3];
				Если ВРег(ЧастиИмени[0]) = "ОБРАБОТКА" Тогда
					Объект = Обработки[ЧастиИмени[1]].Создать();
				ИначеЕсли ВРег(ЧастиИмени[0]) = "ОТЧЕТ" Тогда
					Объект = Отчеты[ЧастиИмени[1]].Создать();
				ИначеЕсли ВРег(ЧастиИмени[0]) = "ВНЕШНЯЯОБРАБОТКА"
				      Или ВРег(ЧастиИмени[0]) = "ВНЕШНИЙОТЧЕТ" Тогда
					Продолжить;
				Иначе
					ТекстОшибки = НСтр("ru = 'Некорректное имя вызываемой процедуры'");
				КонецЕсли;
			...
			Если ТекстОшибки = Неопределено Тогда
				Попытка
					СтандартныеПодсистемыСервер.ПроверитьМетодДляВызоваКакПроизвольныйКод(ИмяМетодаВПараметре,
						ТекущееУсловиеПоиска.ВФоне, Объект, ТекстОшибки);
				Исключение
					ИнформацияОбОшибке = ИнформацияОбОшибке();
		...
		Если ТекстОшибки = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		НомерСтроки = СтрЧислоВхождений(Сред(ТекстМодуля, 1, РезультатПоиска.НачальнаяПозиция), Символы.ПС) + 1;
		ПодробноеОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '%1(%2,%3):
			           |%4
			           |----
			           |%5'"),
			ДополнительныеПараметры.ПолноеИмяМодуля,
			XMLСтрока(НомерСтроки),
			XMLСтрока(ПозицияВСтроке),
			НачалоСтроки + Содержание,
			ТекстОшибки);
		ДобавитьОшибку(ДополнительныеПараметры.ОбъектМетаданных,
			ТекстОшибкиНекорректныйВызовПроцедурыКакПроизвольногоКода(),
			ПодробноеОписаниеОшибки);
	КонецЦикла;
	
КонецПроцедуры

Функция ПараметрыИзСтроки(СтрокаПараметров)
	
	ЧастиСтроки = СтрРазделить(СтрокаПараметров, ",");
	Индекс = ЧастиСтроки.Количество();
	Пока Индекс > 1 Цикл
		Индекс = Индекс - 1;
		ТекущаяЧасть = ЧастиСтроки[Индекс];
		Позиция1 = СтрНайти(ТекущаяЧасть, """", НаправлениеПоиска.СКонца);
		Позиция2 = СтрНайти(ТекущаяЧасть, "'", НаправлениеПоиска.СКонца);
		Если Позиция1 = 0 И Позиция2 = 0 Тогда
			Продолжить;
		КонецЕсли;
		Символ = ?(Позиция1 > Позиция2, """", "'");
		ЧислоВхождений = СтрЧислоВхождений(ТекущаяЧасть, Символ);
		Если ЧислоВхождений - Цел(ЧислоВхождений / 2) * 2 = 0 Тогда
			Продолжить;
		КонецЕсли;
		ЧастиСтроки[Индекс - 1] = ЧастиСтроки[Индекс - 1] + "," + ТекущаяЧасть;
		ЧастиСтроки.Удалить(Индекс);
	КонецЦикла;
...
Функция ТекстОшибкиНекорректныйВызовПроцедурыКакПроизвольногоКода()
	Возврат НСтр("ru = 'Некорректный вызов процедуры, как произвольного кода'");
КонецФункции

Процедура ПроверитьИменаУказанныхМетодовМодуля(ТекстМодуля, ИмяФайла, ДополнительныеПараметры)
	
	ОбъектМетаданных = ДополнительныеПараметры.ОбъектМетаданных;
	
	Если ВРег(ИмяФайла) <> ВРег("ObjectModule")
	   И ВРег(ИмяФайла) <> ВРег("ManagerModule")
	   И Не Метаданные.ОбщиеМодули.Содержит(ОбъектМетаданных)
	 Или СтрНайти(ТекстМодуля, "ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод") = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ПолноеИмя = ОбъектМетаданных.ПолноеИмя();
	
	Если ВРег(ИмяФайла) = ВРег("ObjectModule") Тогда
		МодульМенеджера = ОбщегоНазначения.МенеджерОбъектаПоПолномуИмени(ПолноеИмя);
		Попытка
			Модуль = МодульМенеджера.СоздатьОбъект();
		Исключение
			Возврат;
		КонецПопытки;
		
	ИначеЕсли ВРег(ИмяФайла) = ВРег("ManagerModule") Тогда
		Модуль = ОбщегоНазначения.МенеджерОбъектаПоПолномуИмени(ПолноеИмя);
		
	ИначеЕсли Метаданные.ОбщиеМодули.Содержит(ОбъектМетаданных) Тогда
		Модуль = ОбщегоНазначения.ОбщийМодуль(ОбъектМетаданных.Имя);
	Иначе
		Возврат;
	КонецЕсли;
	...
	// Поиск всех строк с указанными элементами поиска (объявления процедур и функций).
	Шаблон = "(?:^|\s+)(?:%1)\s*\(\s*((?:[^'""()]|'[^']*'|""[^""]*"")*)\s*\)";
	// Пример: (?:^|\s+)(?:Функция\s*ИмяМетода|Процедура\s*ИмяМетода)\s*\(\s*((?:[^'"()]|'[^']*'|"[^"]*")*)\s*\)
	
	МетодыДляПроверки = Новый Соответствие;
	ЭлементыПоиска = Новый Массив;
	Для Каждого ОписаниеМетода Из Методы Цикл
		ИмяМетода = ОписаниеМетода.Ключ;
		МетодыДляПроверки.Вставить(ВРег(ИмяМетода), ИмяМетода);
		ЭлементыПоиска.Добавить("Функция\s*" + ИмяМетода);
		ЭлементыПоиска.Добавить("Процедура\s*" + ИмяМетода);
	КонецЦикла;
	ВыражениеПоиска = СтрШаблон(Шаблон, СтрСоединить(ЭлементыПоиска, "|"));
	...
	Для Каждого РезультатПоиска Из РезультатыПоиска Цикл
		...
		КонецЕсли;
		ИмяМетода = СокрЛП(Сред(ИмяМетода, 1, ПозицияСкобки - 1));
		МетодыДляПроверки.Удалить(ВРег(ИмяМетода));
	КонецЦикла;
	
	Для Каждого КлючИЗначение Из МетодыДляПроверки Цикл
		ПодробноеОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '%1: не найдена процедура или функция %2, указанная в процедуре %3.'"),
			ДополнительныеПараметры.ПолноеИмяМодуля,
			КлючИЗначение.Значение,
			"ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод");
		ДобавитьОшибку(ДополнительныеПараметры.ОбъектМетаданных,
			НСтр("ru = 'Некорректное имя процедуры, вызываемой как произвольный код'"),
			ПодробноеОписаниеОшибки);
	КонецЦикла;
	
КонецПроцедуры

Процедура ПроверитьВызовыМетодовОбработчиковПодсистем(ИменаПроцедур, ЗаголовокОшибки)
	
	Для Каждого ОписаниеИмениПроцедуры Из ИменаПроцедур Цикл
		ИмяПроцедуры = ОписаниеИмениПроцедуры.Ключ;
		ЧастиИмени = СтрРазделить(ИмяПроцедуры, ".");
		Если ЧастиИмени.Количество() = 2 Тогда
			ОбъектМетаданных = Метаданные.ОбщиеМодули.Найти(ЧастиИмени[0]);
		ИначеЕсли ЧастиИмени.Количество() = 3 Тогда
			Попытка
				МодульМенеджера = ОбщегоНазначения.ОбщийМодуль(ЧастиИмени[0] + "." + ЧастиИмени[1]);
			Исключение
				...
				ВызватьИсключение;
			КонецПопытки;
			ОбъектМетаданных = Метаданные.НайтиПоТипу(ТипЗнч(МодульМенеджера));
		Иначе
			Продолжить;
		КонецЕсли;
		Если ОбъектМетаданных = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		ВФоне = ОписаниеИмениПроцедуры.Значение = Истина;
		ТекстОшибки = Неопределено;
		Попытка
			СтандартныеПодсистемыСервер.ПроверитьМетодДляВызоваКакПроизвольныйКод(ИмяПроцедуры, ВФоне,, ТекстОшибки);
		Исключение
			ИнформацияОбОшибке = ИнформацияОбОшибке();
			ТекстОшибки = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
		КонецПопытки;
		...
		ДобавитьОшибку(ОбъектМетаданных,
			ТекстОшибкиНекорректныйВызовПроцедурыКакПроизвольногоКода(),
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ЗаголовокОшибки, ИмяПроцедуры)
...
Процедура ВариантыОтчетов_ПроверитьИспользованиеУстаревшихСвойств(СтрокаОтчет, ОтчетОбъект)
	...
	Исключение
		...
		Подробно = СтрЗаменить(ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()), Символы.ПС, Символы.ПС + "    ");
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки,
...
Функция ДобавитьВидКИВТаблицуВидов(ПредопределенныйВид, ИмяПредопределенного, ТаблицаВидов)
	...
	Если ЗначениеЗаполнено(ПредопределенныйВид.Родитель) Тогда
		ИмяПредопределенногоРодителя = ?(ЗначениеЗаполнено(ПредопределенныйВид.ИмяПредопределенныхДанныхРодителя),
			ПредопределенныйВид.ИмяПредопределенныхДанныхРодителя,ПредопределенныйВид.ИмяПредопределенногоВидаРодителя);
		
		НайденнаяСтрока = ТаблицаВидов.Найти(ИмяПредопределенногоРодителя, "ПредопределенныйВид");
		Если НайденнаяСтрока = Неопределено Тогда
			НоваяСтрока = ТаблицаВидов.Добавить();
			НоваяСтрока.ПредопределенныйВид = ИмяПредопределенногоРодителя;
	...
	Если Не ЭтоСправочник И Не ЭтоДокумент Тогда
		...
		Результат.ЗаголовокОшибки = НСтр("ru = 'Некорректное имя предопределенного вида контактной информации'");
		Возврат Результат;
	КонецЕсли;
	...
	Если МетаданныеВладельца = Неопределено Тогда
		...
		Результат.ЗаголовокОшибки = НСтр("ru = 'Отсутствует объект метаданных'");
		Возврат Результат;
	КонецЕсли;
	
	НайденнаяСтрока = ТаблицаВидов.Найти(ИмяПредопределенного, "ПредопределенныйВид");
	Если НайденнаяСтрока = Неопределено Тогда
		НоваяСтрока = ТаблицаВидов.Добавить();
		НоваяСтрока.ПредопределенныйВид = ИмяПредопределенного;
	...
	Для Каждого Подсистема Из Метаданные.Подсистемы.СтандартныеПодсистемы.Подсистемы Цикл
		Если Подсистема.Подсистемы.Количество() > 0 Тогда
			Для Каждого ПодчиненнаяПодсистема Из Подсистема.Подсистемы Цикл
				Если ПодчиненнаяПодсистема.Имя = "ОбменДаннымиВМоделиСервиса" Тогда
					Продолжить;
				КонецЕсли;
				ДобавитьОбъектыПодсистемы(ПодчиненнаяПодсистема, ВсеОбъектыМетаданных);
			КонецЦикла;
		КонецЕсли;
		Если Подсистема.Имя = "ОбменДанными" Тогда
			Продолжить;
		КонецЕсли;
...
Функция ОбъектыИсключенияПланаОбменаЛюбогоРИБ()
	...
	ДобавитьОбъект("Константа.РегистрироватьПоказателиСерверныхОповещений", Исключения);
	...
	ДобавитьОбъект("РегистрСведений.СвойстваРасширений", Исключения);
	...
	ДобавитьОбъект("РегистрСведений.СнимкиОтчетов", Исключения);
	...
	// СтандартныеПодсистемы.КонтрольРаботыПользователей
	ДобавитьОбъект("Константа.НастройкиРегистрацииСобытийДоступаКДанным", Исключения);
	ДобавитьОбъект("Константа.РегистрироватьИзмененияПравДоступа", Исключения);
	...
	ДобавитьОбъект("Справочник.НастройкиТранспортаСообщенийОбмена", Исключения);
	...
	ДобавитьОбъект("РегистрСведений.УдалитьНастройкиТранспортаОбменаДанными", Исключения);
	...
	ДобавитьОбъект("РегистрСведений.АрхивСообщенийОбменов", Исключения);
	ДобавитьОбъект("РегистрСведений.ЗадачиОбменаДаннымиВнутренняяПубликация", Исключения);
	ДобавитьОбъект("РегистрСведений.КонтурСинхронизации", Исключения);
	ДобавитьОбъект("РегистрСведений.НастройкиАрхиваСообщенийОбменов", Исключения);
	ДобавитьОбъект("РегистрСведений.ОбъектыНезарегистрированныеПриЗацикливании", Исключения);
	...
	ДобавитьОбъект("РегистрСведений.ФиксацияОбработкиДанныхОбработчиками", Исключения);
	...
	ДобавитьОбъект("РегистрСведений.СпискиОтзываСертификатов", Исключения);
...
Функция ДополнительныеОбъектыИсключенияПланаОбменаАРМ()
	...
	ДобавитьВсеОбъектыПодсистемы("КонтрольРаботыПользователей", Исключения);
	ДобавитьВсеОбъектыПодсистемы("ОбновлениеКонфигурации", Исключения);
...
Функция ОбъектыТолькоДляНачальногоОбразаЛюбогоРИБ()
	...
	ДобавитьОбъект("Константа.РазрешитьДоступКИнтернетСервисам", Объекты);
	...
	ДобавитьОбъект("Константа.ПорядокОбрабатываемыхДанных", Объекты);
	...
	// СтандартныеПодсистемы.Пользователи
	ДобавитьОбъект("РегистрСведений.ИерархияГруппПользователей", Объекты);
	ДобавитьОбъект("РегистрСведений.СоставыГруппПользователей", Объекты);
	
	// СтандартныеПодсистемы.РаботаСФайлами
	ДобавитьОбъект("РегистрСведений.УдалитьДвоичныеДанныеФайлов", Объекты);
	ДобавитьОбъект("РегистрСведений.УдалитьХранимыеФайлыВерсий", Объекты);
	ДобавитьОбъект("РегистрСведений.ХранилищеФайлов", Объекты);
	ДобавитьОбъект("Справочник.ХранилищеДвоичныхДанных", Объекты);
	...
	ДобавитьОбъект("РегистрСведений.ГруппыЗначенийДоступа", Объекты);
	...
	// СтандартныеПодсистемы.ЭлектроннаяПодпись
	ДобавитьОбъект("Константа.РазрешитьДоступКИнтернетСервисамЭлектроннойПодписи", Объекты);
...
Процедура ПроверитьОтложенныеОбработчикиНаКорректностьСвойств()
	...
	Для Каждого ИмяПодсистемы Из ОписанияПодсистем.Порядок Цикл
		...
		Для Каждого Обработчик Из ОтложенныеОбработчики Цикл
			...
			Если ЗначениеЗаполнено(ВыявленныеОшибки) Тогда
				ПолноеИмяОбъекта = ПолноеИмяОбъектаМетаданныхИзПроцедурыОбработчикаОбновления(Обработчик.Процедура);
				...
				ДобавитьОшибку(ОбщегоНазначения.ОбъектМетаданныхПоПолномуИмени(ПолноеИмяОбъекта),
...
Процедура ПроверитьПараллельныйОтложенныйОбработчик(Знач Обработчик, Знач СоставПланаОбмена, ОбработчикиДляПроверки, ВыявленныеОшибки, ИмяПодсистемы)
	...
	ПолноеИмяОбъектаОбработчикаОбновления = ПолноеИмяОбъектаМетаданныхИзПроцедурыОбработчикаОбновления(Обработчик.Процедура);
	МассивЭлементовПутиОбъектаОбработчика = СтрРазделить(ПолноеИмяОбъектаОбработчикаОбновления, ".");
	МетаданныеСодержащиеДанные            = МетаданныеСодержащиеДанные();
	
	ТипМетаданныхОбъектаОбработчика = МассивЭлементовПутиОбъектаОбработчика[0];
	ОбработчикВОбновляемойТаблице   = МетаданныеСодержащиеДанные.Найти(ТипМетаданныхОбъектаОбработчика) <> Неопределено;
	
	Если ОбщегоНазначения.ОбъектМетаданныхПоПолномуИмени(ПолноеИмяОбъектаОбработчикаОбновления) = Неопределено Тогда
		
		ТекстОшибки = НСтр("ru = 'У отложенного обработчика ""%1"" некорректно заполнено свойство ""%2"":'");
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, Обработчик.Процедура, "Процедура")
			+ Символы.ПС
			+ НСтр("ru = '- не найден объект'") + " " + ПолноеИмяОбъектаОбработчикаОбновления;
		ЗарегистрироватьОшибкуОбработчика(ВыявленныеОшибки, ТекстОшибки, Обработчик.Процедура);
		
	Иначе
		
		Если ОбработчикВОбновляемойТаблице Тогда
			
			СостояниеРегистрации = СоставПланаОбмена[ПолноеИмяОбъектаОбработчикаОбновления];
			Если СостояниеРегистрации = Неопределено Тогда
				ТекстОшибки = НСтр("ru = 'Объект ""%1"", содержащий процедуру отложенного обработчика ""%2"", не входит
					|в состав плана обмена %3.'");
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, ПолноеИмяОбъектаОбработчикаОбновления,
					Обработчик.Процедура, "ОбновлениеИнформационнойБазы");
				ЗарегистрироватьОшибкуОбработчика(ВыявленныеОшибки, ТекстОшибки, Обработчик.Процедура);
			ИначеЕсли СостояниеРегистрации = АвтоРегистрацияИзменений.Разрешить Тогда
				ТекстОшибки = НСтр("ru = 'Для объекта ""%1"", содержащего отложенный обработчик ""%2"", некорректно
					|установлено свойство %3 в плане обмена %4 (должно быть ""Запрещать"").'");
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, ПолноеИмяОбъектаОбработчикаОбновления,
					Обработчик.Процедура, "АвтоРегистрация", "ОбновлениеИнформационнойБазы");
				ЗарегистрироватьОшибкуОбработчика(ВыявленныеОшибки, ТекстОшибки, Обработчик.Процедура);
			КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(Обработчик.ЧитаемыеОбъекты) И Не ОбработчикВОбновляемойТаблице Тогда
	...
	Иначе
		...
		Для Каждого ЧитаемыйОбъект Из ЧитаемыеОбъекты Цикл
			ОбъектНеСуществует = ОбщегоНазначения.ОбъектМетаданныхПоПолномуИмени(ЧитаемыйОбъект) <> Неопределено;
	...
	Если Не ЗначениеЗаполнено(Обработчик.ИзменяемыеОбъекты) Тогда
		...
		ЗарегистрироватьОшибкуОбработчика(ВыявленныеОшибки, ТекстОшибки, Обработчик.Процедура);
	Иначе
		ИзменяемыеОбъекты = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(Обработчик.ИзменяемыеОбъекты, 
			",", , Истина);
		Для Каждого ИзменяемыйОбъект Из ИзменяемыеОбъекты Цикл
			Если СтрСравнить(ИзменяемыйОбъект, "ПользовательСистемыВзаимодействия") = 0 Тогда
				Продолжить;
			КонецЕсли;
			ОбъектНеСуществует = ОбщегоНазначения.ОбъектМетаданныхПоПолномуИмени(ИзменяемыйОбъект) <> Неопределено;
			Если Не ОбъектНеСуществует Тогда
				НесуществующиеОбъекты = НесуществующиеОбъекты + ?(НесуществующиеОбъекты = "", "", Символы.ПС)
					+ НСтр("ru = '- несуществующий объект метаданных'") + " " + ИзменяемыйОбъект;
				Продолжить;
			КонецЕсли;
...
Функция СоответствиеТиповМетаданных(ИмяМенеджера)
	...
	Если СоответствиеТиповМетаданных = Неопределено Тогда
		СоответствиеТиповМетаданных = Новый Соответствие;
		СоответствиеТиповМетаданных.Вставить("Константы", "Константа");
		СоответствиеТиповМетаданных.Вставить("Справочники", "Справочник");
		СоответствиеТиповМетаданных.Вставить("Документы", "Документ");
		СоответствиеТиповМетаданных.Вставить("ЖурналыДокументов", "ЖурналДокументов");
		СоответствиеТиповМетаданных.Вставить("Перечисления", "Перечисление");
		СоответствиеТиповМетаданных.Вставить("Отчеты", "Отчет");
		СоответствиеТиповМетаданных.Вставить("Обработки", "Обработка");
		СоответствиеТиповМетаданных.Вставить("ПланыВидовХарактеристик", "ПланВидовХарактеристик");
		СоответствиеТиповМетаданных.Вставить("ПланыСчетов", "ПланСчетов");
		СоответствиеТиповМетаданных.Вставить("ПланыОбмена", "ПланОбмена");
		СоответствиеТиповМетаданных.Вставить("ПланыВидовРасчета", "ПланВидовРасчета");
		СоответствиеТиповМетаданных.Вставить("РегистрыСведений", "РегистрСведений");
		СоответствиеТиповМетаданных.Вставить("РегистрыНакопления", "РегистрНакопления");
		СоответствиеТиповМетаданных.Вставить("РегистрыБухгалтерии", "РегистрБухгалтерии");
		СоответствиеТиповМетаданных.Вставить("РегистрыРасчета", "РегистрРасчета");
		СоответствиеТиповМетаданных.Вставить("БизнесПроцессы", "БизнесПроцесс");
		СоответствиеТиповМетаданных.Вставить("Задачи", "Задача");
	КонецЕсли;

	// Поиск по ключу.
	Результат = СоответствиеТиповМетаданных[ИмяМенеджера];
	Если Результат <> Неопределено Тогда
		Возврат Результат;
	КонецЕсли;
	
	// Поиск по ключу и значению без учета регистра.
	Для Каждого Элемент Из СоответствиеТиповМетаданных Цикл
		Если СтрСравнить(Элемент.Ключ, ИмяМенеджера) = 0 Тогда
			Возврат Элемент.Значение;
		КонецЕсли;
		Если СтрСравнить(Элемент.Значение, ИмяМенеджера) = 0 Тогда
...
Функция МетаданныеСодержащиеДанные()
	
	МетаданныеСодержащиеДанные = Новый Массив;
	МетаданныеСодержащиеДанные.Добавить("Константа");
	МетаданныеСодержащиеДанные.Добавить("Справочник");
	МетаданныеСодержащиеДанные.Добавить("Документ");
	МетаданныеСодержащиеДанные.Добавить("ПланВидовХарактеристик");
	МетаданныеСодержащиеДанные.Добавить("ПланСчетов");
	МетаданныеСодержащиеДанные.Добавить("ПланВидовРасчета");
	МетаданныеСодержащиеДанные.Добавить("РегистрСведений");
	МетаданныеСодержащиеДанные.Добавить("РегистрНакопления");
	МетаданныеСодержащиеДанные.Добавить("РегистрБухгалтерии");
	МетаданныеСодержащиеДанные.Добавить("РегистрРасчета");
	МетаданныеСодержащиеДанные.Добавить("БизнесПроцесс");
	МетаданныеСодержащиеДанные.Добавить("Задача");
	
	Возврат МетаданныеСодержащиеДанные;
	
КонецФункции

Функция ПолноеИмяОбъектаМетаданныхИзПроцедурыОбработчикаОбновления(ИмяПроцедуры)
	
	ИмяПроцедурыЧастями = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(ИмяПроцедуры, ".");
...
Процедура ПроверитьОтложенныеОбработчикиНаНаличиеНеразделенныхИзменяемыхОбъектов()
	
	// АПК:326-выкл Без фиксации транзакции, для целей тестирования.
	НачатьТранзакцию();
	Попытка
		КраткоеОписаниеОшибки = НСтр("ru = 'Некорректный отложенный обработчик обновления'");
		
		СоставРазделяемыхДанных = Новый Соответствие;
		ЗаполнитьСоставРазделяемыхДанныхПоОбщимРеквизитам(СоставРазделяемыхДанных);
		
		ПереключитьРазделение(Истина);
		
		ОбработчикиОбновления = ИтерацииОбновления();
		Если ОбработчикиОбновления = Неопределено Тогда
			Возврат;
		КонецЕсли;
		
		ПереключитьРазделение(Ложь);
		
		Если ОбработчикиОбновления.Количество() = 0 Тогда
			ПодробноеОписаниеОшибки = НСтр("ru = 'Не удалось получить информацию об обработчиках обновления.'");
			ДобавитьОшибку(Неопределено, КраткоеОписаниеОшибки, ПодробноеОписаниеОшибки);
			Возврат;
		КонецЕсли;
		
		МетаданныеСодержащиеДанные = МетаданныеСодержащиеДанные();
		
		ПоляОбработчиковОбновленияДляВыводаВОшибки = ПоляОбработчиковОбновленияДляВыводаВОшибки();
		
		ПараметрыОтбора = Новый Структура;
		ПараметрыОтбора.Вставить("РежимВыполнения", "Отложенно");
		
		ОбработчикиОбновленияОтложенно = ОбработчикиОбновления.Скопировать(ПараметрыОтбора);
		Для Каждого ОбработчикОбновления Из ОбработчикиОбновленияОтложенно Цикл
			
			ИнформацияОбОбработчикеОбновления = СтрокаТаблицыЗначенийВСтроку(ОбработчикОбновления,,, ПоляОбработчиковОбновленияДляВыводаВОшибки);
			
			МассивНеразделенныхИзменяемыхОбъектов = Новый Массив;
			
			МассивИзменяемыхОбъектов = СтрРазделить(ОбработчикОбновления.ИзменяемыеОбъекты, ",", Ложь);
			Для Каждого ИзменяемыйОбъект Из МассивИзменяемыхОбъектов Цикл
				
				ИзменяемыйОбъектСтрока = СокрЛП(ИзменяемыйОбъект);
				ИзменяемыйОбъектМетаданных = ОбщегоНазначения.ОбъектМетаданныхПоПолномуИмени(ИзменяемыйОбъектСтрока);
				Если ИзменяемыйОбъектМетаданных = Неопределено Тогда
					// Если объект метаданных не существует, то пропускаем обработчик.
					// Проверка на существование выполняется в процедуре ПроверитьПараллельныйОтложенныйОбработчик.
					Продолжить;
				КонецЕсли;
				
				Если СоставРазделяемыхДанных[ИзменяемыйОбъектМетаданных] = Неопределено Тогда
					МассивНеразделенныхИзменяемыхОбъектов.Добавить(ИзменяемыйОбъектСтрока);
				КонецЕсли;
				
			КонецЦикла;
			
			ПроцедураОбработчикаОбновления = СокрЛП(ОбработчикОбновления.Процедура);
			ОбъектМетаданныхИзПроцедурыОбновления = ОбъектМетаданныхИзПроцедурыОбработчикаОбновления(ПроцедураОбработчикаОбновления);
			
			Если МассивНеразделенныхИзменяемыхОбъектов.Количество() > 0 Тогда
				
				ПодробноеОписаниеОшибки = НСтр(
				...
				ПодробноеОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					ПодробноеОписаниеОшибки,
					СтрСоединить(МассивНеразделенныхИзменяемыхОбъектов, Символы.ПС),
					ИнформацияОбОбработчикеОбновления);
				
				ДобавитьОшибку(ОбъектМетаданныхИзПроцедурыОбновления, КраткоеОписаниеОшибки, ПодробноеОписаниеОшибки);
				
			КонецЕсли;
			
			МассивЭлементовПроцедурыОбработчикаОбновления = СтрРазделить(ПроцедураОбработчикаОбновления, ".", Ложь);
			
			ТипМетаданныхОбъектаОбработчика = МассивЭлементовПроцедурыОбработчикаОбновления[0];
			Если МетаданныеСодержащиеДанные.Найти(ТипМетаданныхОбъектаОбработчика) = Неопределено Тогда
				// Если объект метаданных не содержит данные, то он не входит в состав разделителей, пропускаем.
				Продолжить;
			КонецЕсли;
			
			ИмяПроцедуры = МассивЭлементовПроцедурыОбработчикаОбновления[МассивЭлементовПроцедурыОбработчикаОбновления.ВГраница()];
			
			// Проверяем, что процедура обработчика обновления находится в модуле менеджера.
			МодульМенеджера = МодульМенеджера(ОбъектМетаданныхИзПроцедурыОбновления);
			ПроцедураМодуля = НайтиПроцедуруМодуля(МодульМенеджера, ИмяПроцедуры);
			Если ПроцедураМодуля = Неопределено Тогда
				// Если процедура обработчика обновления не в модуле менеджера, то пропускаем.
				Продолжить;
			КонецЕсли;
			
			// Если объект из процедуры обработчика обновления является неразделенным, то регистрируем ошибку.
			Если СоставРазделяемыхДанных[ОбъектМетаданныхИзПроцедурыОбновления] = Неопределено Тогда
				...
				ПодробноеОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					ПодробноеОписаниеОшибки,
					ПроцедураОбработчикаОбновления,
					ИнформацияОбОбработчикеОбновления);
...
Функция ОбъектМетаданныхИзПроцедурыОбработчикаОбновления(Знач ПолноеИмяПроцедуры)
	
	Если ПустаяСтрока(ПолноеИмяПроцедуры) Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	ПолноеИмяОбъектаМетаданных = ПолноеИмяОбъектаМетаданныхИзПроцедурыОбработчикаОбновления(ПолноеИмяПроцедуры);
	ПолноеИмяОбъектаМетаданных = СокрЛП(ПолноеИмяОбъектаМетаданных);
...
Функция ИтерацииОбновления()
	...
	ИтерацииОбновления = Новый ТаблицаЗначений;
	ИтерацииОбновления.Колонки.Добавить("Подсистема");
	ИтерацииОбновления.Колонки.Добавить("РежимВыполненияОтложенныхОбработчиков");
	ИтерацииОбновления.Колонки.Добавить("ПараллельноеОтложенноеОбновлениеСВерсии");
	ИтерацииОбновления.Колонки.Добавить("НачальноеЗаполнение");
	ИтерацииОбновления.Колонки.Добавить("Версия");
	ИтерацииОбновления.Колонки.Добавить("Процедура");
	ИтерацииОбновления.Колонки.Добавить("РежимВыполнения");
	ИтерацииОбновления.Колонки.Добавить("ВыполнятьВГруппеОбязательных");
	ИтерацииОбновления.Колонки.Добавить("ОбщиеДанные");
	ИтерацииОбновления.Колонки.Добавить("УправлениеОбработчиками");
	ИтерацииОбновления.Колонки.Добавить("Комментарий");
	ИтерацииОбновления.Колонки.Добавить("Идентификатор");
	ИтерацииОбновления.Колонки.Добавить("ПроцедураПроверки");
	ИтерацииОбновления.Колонки.Добавить("БлокируемыеОбъекты");
	ИтерацииОбновления.Колонки.Добавить("ПроцедураЗаполненияДанныхОбновления");
	ИтерацииОбновления.Колонки.Добавить("ОчередьОтложеннойОбработки");
	ИтерацииОбновления.Колонки.Добавить("ЗапускатьТолькоВГлавномУзле");
	ИтерацииОбновления.Колонки.Добавить("ЗапускатьИВПодчиненномУзлеРИБСФильтрами");
	ИтерацииОбновления.Колонки.Добавить("ЧитаемыеОбъекты");
	ИтерацииОбновления.Колонки.Добавить("ИзменяемыеОбъекты");
	ИтерацииОбновления.Колонки.Добавить("Приоритет");
	ИтерацииОбновления.Колонки.Добавить("МонопольныйРежим");
	...
	Попытка
		...
		Для Каждого ОписаниеПодсистемы Из ОписанияПодсистем Цикл
			...
			Если ОписаниеПодсистемы.Ключ = "БиблиотекаТехнологииСервиса" Тогда
				Продолжить;
			КонецЕсли;
			...
			Модуль = ОбщегоНазначения.ОбщийМодуль(ОсновнойСерверныйМодуль);
			Модуль.ПриДобавленииОбработчиковОбновления(Обработчики);
			...
			Для Каждого Обработчик Из Обработчики Цикл
				...
				НоваяСтрока = ИтерацииОбновления.Добавить();
	...
	Исключение
		...
		КраткоеОписаниеОшибки = НСтр("ru = 'Ошибка при проверке отложенных обработчиков обновления'");
		ТекстОписанияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось получить информацию об обработчиках обновления по причине:
				|%1'"), ПодробноеОписаниеОшибки);
		ДобавитьОшибку(Неопределено, КраткоеОписаниеОшибки, ТекстОписанияОшибки);
		Возврат Неопределено;
	КонецПопытки;
...
Процедура ЗаполнитьСоставРазделяемыхДанныхПоОбщимРеквизитам(СоставРазделяемыхДанных)
	
	РазделениеДанныхОбщегоРеквизитаРазделять = Метаданные.СвойстваОбъектов.РазделениеДанныхОбщегоРеквизита.Разделять;
	
	Для Каждого ОбъектМетаданныхОбщийРеквизит Из Метаданные.ОбщиеРеквизиты Цикл
		Если ОбъектМетаданныхОбщийРеквизит.РазделениеДанных = РазделениеДанныхОбщегоРеквизитаРазделять Тогда
			ЗаполнитьСоставРазделяемыхДанныхПоОбщемуРеквизиту(СоставРазделяемыхДанных, ОбъектМетаданныхОбщийРеквизит);
		КонецЕсли;
	КонецЦикла;
	
КонецПроцедуры

Процедура ЗаполнитьСоставРазделяемыхДанныхПоОбщемуРеквизиту(СоставРазделяемыхДанных, ОбъектМетаданныхОбщийРеквизит)
	
	АвтоИспользованиеОбщегоРеквизита = ОбъектМетаданныхОбщийРеквизит.АвтоИспользование;
	АвтоИспользованиеОбщегоРеквизитаИспользовать = Метаданные.СвойстваОбъектов.АвтоИспользованиеОбщегоРеквизита.Использовать;
	АвтоИспользованиеОбщегоРеквизитаРавноИспользовать = (АвтоИспользованиеОбщегоРеквизита = АвтоИспользованиеОбщегоРеквизитаИспользовать);
	
	ИспользованиеОбщегоРеквизитаИспользовать = Метаданные.СвойстваОбъектов.ИспользованиеОбщегоРеквизита.Использовать;
	ИспользованиеОбщегоРеквизитаАвто = Метаданные.СвойстваОбъектов.ИспользованиеОбщегоРеквизита.Авто;
	
	СоставОбщегоРеквизита = ОбъектМетаданныхОбщийРеквизит.Состав;
	Для Каждого ЭлементСоставаОбщегоРеквизита Из СоставОбщегоРеквизита Цикл
		
		ИспользованиеОбщегоРеквизита = ЭлементСоставаОбщегоРеквизита.Использование;
		Если (ИспользованиеОбщегоРеквизита = ИспользованиеОбщегоРеквизитаИспользовать)
		ИЛИ ((ИспользованиеОбщегоРеквизита = ИспользованиеОбщегоРеквизитаАвто) И АвтоИспользованиеОбщегоРеквизитаРавноИспользовать) Тогда
			СоставРазделяемыхДанных.Вставить(ЭлементСоставаОбщегоРеквизита.Метаданные, Истина);
		КонецЕсли;
...
Функция ПоляОбработчиковОбновленияДляВыводаВОшибки()
	
	ПоляОбработчиковСтрокой =
		"Подсистема
		|РежимВыполненияОтложенныхОбработчиков
		|Версия
		|Процедура
		|Комментарий
		|ЧитаемыеОбъекты
	...
	Возврат СтрРазделить(ПоляОбработчиковСтрокой, Символы.ПС, Ложь);
	
КонецФункции

Функция СтрокаТаблицыЗначенийВСтроку(Знач СтрокаТаблицыЗначений, Знач Разделитель = "",
	Знач ДобавлятьПустыеЗначения = Истина, Знач КолонкиДляВыводаВСтроку = Неопределено)
	
	ТипЗначенияДанных = ТипЗнч(СтрокаТаблицыЗначений);
	Если (ТипЗначенияДанных <> Тип("СтрокаТаблицыЗначений"))
	   И (ТипЗначенияДанных <> Тип("СтрокаДереваЗначений")) Тогда
		Возврат "";
	КонецЕсли;
	...
	Для Каждого Колонка Из ТаблицаЗначений.Колонки Цикл
		...
		Если КолонкиДляВыводаВСтроку <> Неопределено Тогда
			Если КолонкиДляВыводаВСтроку.Найти(ИмяКолонки) = Неопределено Тогда
				Продолжить;
			КонецЕсли;
		КонецЕсли;
		
		ЗначениеЯчейки = СтрокаТаблицыЗначений[ИмяКолонки];
		Если (НЕ ДобавлятьПустыеЗначения) И ПустаяСтрока(ЗначениеЯчейки) Тогда
			Продолжить;
		КонецЕсли;
		
		ЭлементМассива = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("%1: %2", ИмяКолонки, ЗначениеЯчейки);
		МассивЗначений.Добавить(ЭлементМассива);
		
		Если ТипЗнч(ЗначениеЯчейки) = Тип("ТаблицаЗначений") Тогда
			Для Каждого СтрокаВложеннойТаблицыЗначений Из ЗначениеЯчейки Цикл
				ЗначениеВложеннойЯчейки = СтрокаТаблицыЗначенийВСтроку(СтрокаВложеннойТаблицыЗначений);
				МассивЗначений.Добавить(ЗначениеВложеннойЯчейки);
			КонецЦикла;
		КонецЕсли;
		
	КонецЦикла;
	
	Возврат СтрСоединить(МассивЗначений, Разделитель);
...
Функция ПодключаемыеКоманды_НастройкиИсточников()
	...
	НастройкиИсточников.Колонки.Добавить("ПечатьНастройкиПечати",          Новый ОписаниеТипов("Булево"));
	...
	НастройкиИсточников.Колонки.Добавить("НапоминанияПользователя",        Новый ОписаниеТипов("Булево"));
	
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Печать") Тогда
		МодульУправлениеПечатью = ОбщегоНазначения.ОбщийМодуль("УправлениеПечатью");
		МассивМетаданныхВсехИсточников = МодульУправлениеПечатью.ИсточникиКомандПечати();
		НастройкиПечати = МодульУправлениеПечатью.НастройкиПечати();
			
		МассивМетаданныхНастройкиПечати = Новый Массив;
		Для Каждого ОбъектМенеджер Из НастройкиПечати.ОбъектыПечати Цикл
			МассивМетаданныхНастройкиПечати.Добавить(Метаданные.НайтиПоТипу(ТипЗнч(ОбъектМенеджер)));
		КонецЦикла;
		
		МассивМетаданных = ОбщегоНазначенияКлиентСервер.РазностьМассивов(МассивМетаданныхВсехИсточников, МассивМетаданныхНастройкиПечати);
		ЗаполнитьТаблицуЗначений(НастройкиИсточников, МассивМетаданных, "Метаданные", Новый Структура("Печать", Истина));
		ЗаполнитьТаблицуЗначений(НастройкиИсточников, МассивМетаданныхНастройкиПечати, "Метаданные", Новый Структура("ПечатьНастройкиПечати", Истина));
	КонецЕсли;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.НапоминанияПользователя") Тогда
		МассивМетаданных = Новый Массив;
		ОпределяемыеТипыСтрокой = "ПредметНапоминания, ПредметНапоминанияОбъект";
		МассивНаименованийОпределяемыхТипов = СтрРазделить(ОпределяемыеТипыСтрокой, ",", Ложь);
		Для Каждого НаименованиеОпределяемогоТипа Из МассивНаименованийОпределяемыхТипов Цикл
			НаименованиеОпределяемогоТипа = СокрЛП(НаименованиеОпределяемогоТипа);
			МассивОпределяемыхТипов = Метаданные.ОпределяемыеТипы[НаименованиеОпределяемогоТипа].Тип.Типы();
			Для Каждого ОпределяемыйТип Из МассивОпределяемыхТипов Цикл
				ОбъектМетаданных = Метаданные.НайтиПоТипу(ОпределяемыйТип);
				Если ОбъектМетаданных = Неопределено Тогда
					Продолжить;
				КонецЕсли;
				Если МассивМетаданных.Найти(ОбъектМетаданных) = Неопределено Тогда
					МассивМетаданных.Добавить(ОбъектМетаданных);
				КонецЕсли;
			КонецЦикла;
		КонецЦикла;
		ЗаполнитьТаблицуЗначений(НастройкиИсточников, МассивМетаданных, "Метаданные", Новый Структура("НапоминанияПользователя", Истина));
	КонецЕсли;
...
Функция ПодключаемыеКоманды_СведенияОПодключенныхОбъектах()
	...
	Для Каждого ОбъектМетаданных Из Состав Цикл
		...
		Исключение
			...
			Подробно = СокрЛП(ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
			Подробно = Подробно
...
Процедура ПроверитьЛишниеПодпискиПрефиксации()
	...
	Для Каждого ОписаниеОшибки Из ОписанияОшибок Цикл
		...
		ДобавитьОшибку(ОбщегоНазначения.ОбъектМетаданныхПоПолномуИмени(ОписаниеОшибки.ПолноеИмя),
			...
			Если СвойстваМакета.Значение <> СохраненныйМакет Тогда
				...
				Иначе
					...
					Для Каждого ЭлементМассива Из Добавленные Цикл
						...
						ДобавитьОшибку(ОбщегоНазначения.ОбъектМетаданныхПоПолномуИмени(ЭлементМассива), КраткоеОписаниеОшибки, ТекстОшибки);
					КонецЦикла;
...
Процедура ПроверитьСоставСтандартныхРолей()
	...
	Для Каждого ЭлементСостава Из Метаданные.ОбщиеРеквизиты.Найти("ОбластьДанныхОсновныеДанные").Состав Цикл
		...
		ОбъектРазделенный = (Элемент = Неопределено) Или (Элемент <> Неопределено И Элемент.ТекстовоеСодержимое <> "DontUse");
...
Процедура СоставРолиАдминистраторСистемы(РазделенныеОбъектыМетаданных, НеразделенныеОбъектыМетаданных, Исключения)
	...
	Пока Истина Цикл
		...
		Если Не ЭтоДемоБСП() И Не ЭтоОбъектБСП(ОбщегоНазначения.ОбъектМетаданныхПоПолномуИмени(ПолноеИмяОбъекта)) Тогда
			Продолжить;
		КонецЕсли;
		...
		Если ЧастиИмени.Количество() > 2 Тогда
			...
			Иначе
				...
				ОбъектМетаданных = ОбщегоНазначения.ОбъектМетаданныхПоПолномуИмени(ИмяДляОшибки);
				ДобавитьОшибку(ОбъектМетаданных,
		...
		ИначеЕсли НеразделенныеОбъектыМетаданных.Получить(ПолноеИмяОбъекта) = Ложь Тогда
			...
			Для Каждого Право Из ЭлементСостава.ДочерниеУзлы Цикл
				...
				Иначе
					Если ИсправлятьОшибку("УстановленыЗапрещенныеПрава") И ЕстьОшибкиСоставаРазделителей <> Истина Тогда
						...
						ВнесеныИзменения = Истина;
					Иначе
						СписокПрав.Добавить(ИмяПрава);
					КонецЕсли;
				КонецЕсли;
			КонецЦикла;
	
			Если ЗначениеЗаполнено(СписокПрав) Тогда
				Если ЭтоДемоБСП() Тогда
					ШаблонОшибки = НСтр("ru = 'В роли %1 для %2 установлены запрещенные права:
						| %3.
						|Возможно некорректно установлены значения разделителей.'");
				Иначе
					ШаблонОшибки = НСтр("ru = 'В роли %1 для %2 установлены запрещенные права:
				...
				КонецЕсли;
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					ШаблонОшибки, "АдминистраторСистемы", ИмяДляОшибки, СтрСоединить(СписокПрав, Символы.ПС));
				ОбъектМетаданных = ОбщегоНазначения.ОбъектМетаданныхПоПолномуИмени(ИмяДляОшибки);
				ДобавитьОшибку(ОбъектМетаданных,
			...
			Для Каждого Право Из ДопустимыеПрава Цикл
				Если ТипЗнч(Право.Значение) = Тип("Строка") Тогда
					Если ИсправлятьОшибку("НеУстановленыНеобходимыеПрава") И ЕстьОшибкиСоставаРазделителей <> Истина Тогда
						...
						ВнесеныИзменения = Истина;
					Иначе
						СписокПрав.Добавить(Право.Значение);
					КонецЕсли;
				КонецЕсли;
			КонецЦикла;
			...
			Если ЗначениеЗаполнено(СписокПрав) Тогда
				Если ЭтоДемоБСП() Тогда
					ШаблонОшибки = НСтр("ru = 'В роли %1 для %2 не установлены необходимые права:
						|%3.
						|Возможно, некорректно установлены значения разделителей.'");
				Иначе
					ШаблонОшибки = НСтр("ru = 'В роли %1 для %2 не установлены необходимые права:
	...
	Для Каждого Объект Из НеразделенныеОбъектыМетаданных Цикл
		...
		Если Не ЭтоДемоБСП() И Не ЭтоОбъектБСП(ОбщегоНазначения.ОбъектМетаданныхПоПолномуИмени(Объект.Ключ)) Тогда
			Продолжить;
		КонецЕсли;
	...
	Пока Истина Цикл
		...
		Если НеразделенныеОбъектыМетаданных.Получить(ПолноеИмяОбъекта) = Ложь Тогда
			...
			Если ЗначениеЗаполнено(СписокПрав) Тогда
				...
				КонецЕсли;
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					ШаблонОшибки, "ПолныеПрава", ИмяДляОшибки, СтрСоединить(СписокПрав, Символы.ПС));
				ОбъектМетаданных = ОбщегоНазначения.ОбъектМетаданныхПоПолномуИмени(ИмяДляОшибки);
				ДобавитьОшибку(ОбъектМетаданных,
		...
		ИначеЕсли РазделенныеОбъектыМетаданных.Получить(ПолноеИмяОбъекта) = Ложь Тогда
			...
			Для Каждого Право Из ЭлементСостава.ДочерниеУзлы Цикл
				...
				Если ИсправлятьОшибку("НеУстановленыНеобходимыеПрава") И ЕстьОшибкиСоставаРазделителей <> Истина Тогда
					...
					ВнесеныИзменения = Истина;
				Иначе
					СписокПрав.Добавить(РазрешенныеОбъекты[ТипОбъекта][ИмяПрава]);
				КонецЕсли;
			КонецЦикла;
	...
	Для Каждого Объект Из РазделенныеОбъектыМетаданных Цикл
		...
		Для Каждого Право Из ДопустимыеПрава Цикл
			Если РазрешенныеПраваНаРазделенные.Свойство(Право.Ключ) Тогда
				...
				Иначе
					СписокПрав.Добавить(ДопустимыеПрава[Право.Ключ]);
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
...
Процедура ОпределитьВидыОграниченийПрав(Параметры)
	...
	Для Каждого Строка Из ОграниченияДоступа Цикл
		...
		Если СтрСравнить(Свойства.Роль, "ПолныеПрава") = 0
			Или СтрСравнить(Свойства.Роль, "АдминистраторСистемы") = 0 Тогда
			Продолжить;
		КонецЕсли;
		...
		Если СтрСравнить(Свойства.Право, "Добавление") = 0
		 ИЛИ СтрСравнить(Свойства.Право, "Удаление") = 0 Тогда
		...
		Если СтрСравнить(Лев(Ограничение, СтрДлина("#ПоЗначениям(")), "#ПоЗначениям(") = 0 Тогда
			...
			Пока Позиция > 0 Цикл
				...
				Если ЗначениеЗаполнено(ВидДоступа) Тогда
					...
					// @skip-check query-in-loop - устаревший код (стандартный вариант RLS)
		...
		ИначеЕсли (СтрСравнить(Лев(Ограничение, СтрДлина("#ПоЗначениямРасширенный(")), "#ПоЗначениямРасширенный(") = 0) 
			Или (СтрСравнить(Лев(Ограничение, СтрДлина("#ПоЗначениямИНаборамРасширенный(")), "#ПоЗначениямИНаборамРасширенный(") = 0) Тогда
		...
		ИначеЕсли СтрСравнить(Лев(Ограничение, СтрДлина("#ПоНаборамЗначений(")), "#ПоНаборамЗначений(") = 0 Тогда
	...
	ОбъектМетаданныхРегистрСведенийВерсииОбъектов = ОбщегоНазначения.ОбъектМетаданныхПоПолномуИмени("РегистрСведений.ВерсииОбъектов");
	Для каждого Строка Из Ограничения Цикл
		Если СтрСравнить(Строка.Поля, СтрокаПрочиеПоля) <> 0 Тогда
...
Функция НовоеОписаниеВидовОграниченийПрав(ВидыОграниченийПрав)
	...
	Для каждого Строка Из ВидыОграниченийПрав Цикл
		Текст = ВРег(Строка.Таблица);
		Если СтрНачинаетсяС(Текст, ВРег("Справочник.")) Тогда
			Строка.ПорядокКоллекции = 1;
			
		ИначеЕсли СтрНачинаетсяС(Текст, ВРег("Документ.")) Тогда
			Строка.ПорядокКоллекции = 2;
			
		ИначеЕсли СтрНачинаетсяС(Текст, ВРег("ЖурналДокументов.")) Тогда
			Строка.ПорядокКоллекции = 3;
			
		ИначеЕсли СтрНачинаетсяС(Текст, ВРег("ПланВидовХарактеристик.")) Тогда
			Строка.ПорядокКоллекции = 4;
			
		ИначеЕсли СтрНачинаетсяС(Текст, ВРег("ПланСчетов.")) Тогда
			Строка.ПорядокКоллекции = 5;
			
		ИначеЕсли СтрНачинаетсяС(Текст, ВРег("ПланВидовРасчета.")) Тогда
			Строка.ПорядокКоллекции = 6;
			
		ИначеЕсли СтрНачинаетсяС(Текст, ВРег("РегистрСведений.")) Тогда
			Строка.ПорядокКоллекции = 7;
			
		ИначеЕсли СтрНачинаетсяС(Текст, ВРег("РегистрНакопления.")) Тогда
			Строка.ПорядокКоллекции = 8;
			
		ИначеЕсли СтрНачинаетсяС(Текст, ВРег("РегистрБухгалтерии.")) Тогда
			Строка.ПорядокКоллекции = 9;
			
		ИначеЕсли СтрНачинаетсяС(Текст, ВРег("РегистрРасчета.")) Тогда
			Строка.ПорядокКоллекции = 10;
			
		ИначеЕсли СтрНачинаетсяС(Текст, ВРег("БизнесПроцесс.")) Тогда
			Строка.ПорядокКоллекции = 11;
			
		ИначеЕсли СтрНачинаетсяС(Текст, ВРег("Задача.")) Тогда
...
Процедура СравнитьОписанияВидовОграниченийПрав(СуществующееОписание, НовоеОписание)
	...
	Для Каждого ТекущаяОшибкаРасхождения Из ОшибкиРасхождения Цикл
		...
		Если ЗначениеЗаполнено(ТекущаяОшибкаРасхождения.ТекстОшибкиНедостающихОграничений)
			...
			ДобавитьОшибку(ОбщегоНазначения.ОбъектМетаданныхПоПолномуИмени(ТекущаяОшибкаРасхождения.ПолноеИмяОМ), ЗаголовокОшибки, ТекстОшибки);
...
Процедура ДобавитьОграниченияПравРоли(ОграниченияПрав, Роль)
	...
	Для Каждого ОписаниеОбъекта Из Объекты Цикл
		Если СтрЧислоВхождений(ОписаниеОбъекта.Ключ, ".") <> 1 Тогда
			Продолжить;
		КонецЕсли;
		ОбъектМетаданных = ОбщегоНазначения.ОбъектМетаданныхПоПолномуИмени(ОписаниеОбъекта.Ключ);
		Если ОбъектМетаданных = Неопределено Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
Процедура УправлениеДоступом_ПроверитьВставкуТекстаВМодулеФормы(Параметры, ОбъектМетаданныхФорма)
	...
	Если ЗначениеЗаполнено(Модуль.Структура) Тогда
		ОчиститьПеременнуюМодульФормы(Модуль.Структура.Содержимое);
		Модуль = Неопределено;
	КонецЕсли;
...
Процедура УправлениеДоступом_ПроверитьИспользованиеПредопределенныхВМодулеФормы(ОбъектМетаданныхФорма)
	...
	Модуль = МодульФормы(ОбъектМетаданныхФорма);
	УправлениеДоступом_ПроверитьИспользованиеПредопределенныхМодулей(ОбъектМетаданныхФорма,
		Модуль,
...
Процедура УправлениеДоступом_ПроверитьИспользованиеПредопределенныхВМодулеКоманды(ОбъектМетаданныхКоманда)
	...
	УправлениеДоступом_ПроверитьИспользованиеПредопределенныхМодулей(ОбъектМетаданныхКоманда,
		Модуль,
...
Процедура УправлениеДоступом_ПроверитьИспользованиеПредопределенныхВОбщемМодуле(ОбъектМетаданных)
	...
	УправлениеДоступом_ПроверитьИспользованиеПредопределенныхМодулей(ОбъектМетаданных,
		Модуль,
...
Функция ВхожденияСтроки(ТекстМодуля, СтрокаПоиска, ПроверитьЧтоСледующийСимволИмениПеременной = Ложь)
	...
	ВыражениеДляПоиска = СтрокаПоиска;
	ВыражениеДляПоиска = СтрЗаменить(ВыражениеДляПоиска, "(", "\(");
	ВыражениеДляПоиска = СтрЗаменить(ВыражениеДляПоиска, ")", "\)");
	...
	Пока Истина Цикл
		РезультатПоиска = СтрНайтиПоРегулярномуВыражению(ТекстМодуля, ВыражениеДляПоиска, 
			НаправлениеПоиска.СНачала, Позиция + 1);
		Если РезультатПоиска = Неопределено Тогда
			Прервать;
		КонецЕсли;
		
		Позиция = РезультатПоиска.НачальнаяПозиция;
		Если Позиция = 0 Тогда
			Прервать;
		КонецЕсли;
		
		НомерСтроки = ПолучитьНомерСтрокиМодуля(ТекстМодуля, Позиция);
		Строка = СтрПолучитьСтроку(ТекстМодуля, НомерСтроки);
		
		Если ПроверитьЧтоСледующийСимволИмениПеременной Тогда
			ПозицияОкончания = СтрНайти(Строка, СтрокаПоиска) + РезультатПоиска.Длина;
			Если СтрДлина(Строка) > ПозицияОкончания И Не ЭтоСимволИмениПеременной(Сред(Строка, ПозицияОкончания, 1)) Тогда
...
Процедура ПроверитьТипыИзмеренийРегистровКлючей(Параметры)
	...
	Для Каждого ОписаниеРегистра Из ТипыИзмеренийРегистровКлючей Цикл
		...
		ОбъектМетаданных = ОбщегоНазначения.ОбъектМетаданныхПоПолномуИмени(ИмяРегистра);
		Если ОбъектМетаданных = Неопределено Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ОбновитьСписокПредопределенныхЭлементовСправочника(ИмяСправочника, ОбязательныеЭлементы)
	...
	Для Каждого ОписаниеПредопределенного Из СписокПредопределенных Цикл
		...
		ОбъектМетаданных = ОбщегоНазначения.ОбъектМетаданныхПоПолномуИмени(ОписаниеПредопределенного.Значение);
...
Процедура ПроверитьИспользованиеНовогоRLSВРолях(ОграниченияДоступа, СпискиСОграничениемДоступа)
	...
	Для Каждого Таблица Из СписокТаблиц Цикл
		...
		ОбъектМетаданных = ОбщегоНазначения.ОбъектМетаданныхПоПолномуИмени(Таблица);
...
#Область ПроверкаМодулей

Функция НайтиПроцедуруМодуля(Модуль, ИмяПроцедуры)
	
	ТекстМодуля = Модуль.ТекстМодуля;
	Если ТекстМодуля = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	// Поиск процедуры с нужным именем в начале строки.
	ВыражениеДляПоиска  = "(?m)^Процедура " + ИмяПроцедуры + "\(";
	
	РезультатПоискаНачалоПроцедуры = СтрНайтиПоРегулярномуВыражению(ТекстМодуля, ВыражениеДляПоиска, 
		НаправлениеПоиска.СНачала, 1);
	Если РезультатПоискаНачалоПроцедуры = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	НачальнаяПозиция = РезультатПоискаНачалоПроцедуры.НачальнаяПозиция;
	Если НачальнаяПозиция = 0 Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	РезультатПоискаКонецПроцедуры = Неопределено;
	
	// Поиск в начале строки слова "КонецПроцедуры" после объявления найденной процедуры.
	ВыражениеДляПоиска = "(?m)^КонецПроцедуры";
	РезультатПоискаКонецПроцедуры = СтрНайтиПоРегулярномуВыражению(ТекстМодуля, ВыражениеДляПоиска, 
		НаправлениеПоиска.СНачала, НачальнаяПозиция);
	
	Если РезультатПоискаКонецПроцедуры.НачальнаяПозиция = 0 Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	Результат = Новый Структура;
	Результат.Вставить("ТекстПроцедуры",          "");
	...
	// Получение текста процедуры.
	СтрДлинаПроцедуры = РезультатПоискаКонецПроцедуры.НачальнаяПозиция + СтрДлина("КонецПроцедуры") - НачальнаяПозиция;
	Результат.ТекстПроцедуры = Сред(ТекстМодуля, НачальнаяПозиция, СтрДлинаПроцедуры);
	
	МассивСтрок = СтрРазделить(Результат.ТекстПроцедуры, Символы.ПС);
	Если МассивСтрок.Количество() = 0 Тогда
		Возврат Результат;
	КонецЕсли;
	
	// Заголовок - первая строка процедуры.
	Результат.Заголовок = МассивСтрок[0];
	
	// Содержимое - текст внутри процедуры.
	МассивСтрок.Удалить(0);
	МассивСтрок.Удалить(МассивСтрок.ВГраница());
	Результат.Содержимое = МассивСтрок;
...
Функция ОписаниеМодуля(ПолноеИмяМодуля)
	...
	Результат.Вставить("Структура", Неопределено);
	Возврат Результат;
КонецФункции
...
Процедура УправлениеДоступом_ПроверитьИспользованиеПредопределенныхМодулей(ОбъектМетаданных, Модуль, ШаблонОшибки, ИмяОбъектаМетаданных)
...
Процедура ВыгрузитьКонфигурациюВXML(КаталогВыгрузкиКонфигурации)
	...
	Если Результат.КодВозврата <> 0 Тогда
		...
		Если ЗначениеЗаполнено(Результат.ПотокОшибок) Тогда
			Сообщения.Добавить(Результат.ПотокОшибок);
		КонецЕсли;
		Если ЗначениеЗаполнено(Результат.ПотокВывода) Тогда
			Сообщения.Добавить(Результат.ПотокВывода);
		КонецЕсли;
		
		Попытка
			
			ПроверкаНаличияФайла = Новый Файл(ИмяФайлаСообщений);
			Если ПроверкаНаличияФайла.Существует() Тогда
				
				ЧтениеТекста = Новый ЧтениеТекста(ИмяФайлаСообщений, КодировкаТекста.UTF8);
				Сообщения.Добавить(ЧтениеТекста.Прочитать());
				ЧтениеТекста.Закрыть();
				
			КонецЕсли; 
			
			Если Сообщения.Количество() = 0 Тогда
				Сообщения.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Код возврата запуск системы 1С:Предприятие в режиме Конфигуратора: %1'"), Результат.КодВозврата));
			КонецЕсли;
			
		Исключение
			Сообщения.Добавить(ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецПопытки;
		
		ШаблонТекстаОшибки = НСтр("ru = '%1
		...
		СтрСоединить(Сообщения, Символы.ПС), Результат.КодВозврата, СтрСоединить(КомандаЗапуска, " "));
		
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Проверка внедрения БСП'", ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,, ТекстОшибки);
		
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Не удалось выполнить выгрузку конфигурации в файлы по причине:
		|%1'"), СтрСоединить(Сообщения, Символы.ПС));
	КонецЕсли;
...
Процедура ЗагрузитьКонфигурациюИзXML()
	...
	ИмяФайлаСообщений = КаталогВыгрузки + "СообщенияВыгрузкиКонфигурацииВФайлы.txt";
	КаталогПрограммы = СтандартныеПодсистемыСервер.ПараметрыКлиентаНаСервере().Получить("КаталогПрограммы");
...
Функция РезультатПроверки(ПараметрыПроверки)
	...
	РезультатСтрокой = ПараметрыПроверки.РезультатСтрокой;
	
	Если ЗначениеЗаполнено(ПараметрыПроверки.РасширениеФайлаПроверки) Тогда
	...
	ИначеЕсли ЗначениеЗаполнено(ПараметрыПроверки.ПолныйПутьКФайлуПроверки) Тогда
...
Процедура ЗаполнитьДанныеДляПроверки(ПараметрыПроверки)
	...
	Если ПараметрыПроверки <> Неопределено И ПараметрыПроверки.ИсправлятьОшибки Тогда
	...
	ИнтерактивныйЗапуск = ПараметрыПроверки <> Неопределено И ПараметрыПроверки.ИнтерактивныйЗапуск;
	
	Если ПараметрыПроверки <> Неопределено И ПараметрыПроверки.ТаблицаИсключений <> Неопределено Тогда
...
Процедура ИсключитьТипы(ИсключаемыеТипы, МассивМетаданных)
	...
	Если Не ПустаяСтрока(ИсключаемыеТипы) Тогда
		...
		Для Каждого ИсключаемыйТип Из МассивИсключаемыхТипов Цикл
			...
			ОбъектМетаданных = ОбщегоНазначения.ОбъектМетаданныхПоПолномуИмени(ИсключаемыйТип);
...
Процедура ЗаполнитьОбъектыПодсистем(Подсистема, ЭтоПодсистемаБСП = Неопределено)
	...
	Для Каждого Объект Из Подсистема.Состав Цикл
		...
		ДанныеПодсистемы = Новый Структура("Имя, Представление, Интерфейсная",
			ПодсистемаКонфигурации.Имя,
			ПодсистемаКонфигурации.Представление(),
			ПодсистемаКонфигурации.ВключатьВКомандныйИнтерфейс);
	...
	Для Каждого ПодчиненнаяПодсистема Из Подсистема.Подсистемы Цикл
		Если ПодчиненнаяПодсистема.Имя = "ОбменДанными"
			Или ПодчиненнаяПодсистема.Имя = "ОбменДаннымиВМоделиСервиса" Тогда
			ЗаполнитьОбъектыПодсистем(ПодчиненнаяПодсистема, Ложь);
		Иначе
			...
			Для Каждого Форма Из ОбъектМетаданных.Формы Цикл // ОбъектМетаданныхФорма
				...
				Если ЭтоФормаСписка Тогда
					...
					Если ИмяОсновнойТаблицыСпискаФормы <> Неопределено Тогда
						ОбъектМетаданныхФормы = ОбщегоНазначения.ОбъектМетаданныхПоПолномуИмени(ИмяОсновнойТаблицыСпискаФормы);
					КонецЕсли;
				КонецЕсли;
...
Функция СвойстваОбъектаПоИмениФайла(Знач ИмяФайла)
	...
	ПолноеИмяОбъектаМетаданных = МассивИмени[0];
	Если МассивИмени.Количество() > 1 Тогда
		ПолноеИмяОбъектаМетаданных = ПолноеИмяОбъектаМетаданных + "." + МассивИмени[1];
	КонецЕсли;
	
	СвойстваОбъекта = Новый Структура;
	СвойстваОбъекта.Вставить("Представление", СтрСоединить(МассивИмени, "."));
	СвойстваОбъекта.Вставить("ОбъектМетаданных", ОбщегоНазначения.ОбъектМетаданныхПоПолномуИмени(ПолноеИмяОбъектаМетаданных));
...
Процедура ВыполнитьПроверку(ПараметрыПроверки = Неопределено)
	...
	Если ИсправлятьОшибки <> Неопределено Тогда
		ПараметрыПроверки.ИсправлятьОшибки = ИсправлятьОшибки;
	КонецЕсли;
	Если ИсправляемыеОшибки <> Неопределено Тогда
		ПараметрыПроверки.ИсправляемыеОшибки = ИсправляемыеОшибки;
	КонецЕсли;
	Если ОтборПоПодсистемам <> Неопределено Тогда
		ПараметрыПроверки.ПроверяемыеПодсистемы = ОтборПоПодсистемам;
	КонецЕсли;
	
	ПараметрыПроверки.Вставить("ИнтерактивныйЗапуск", Ложь);
	Если ИнтерактивныйЗапуск = Истина Тогда
		ПараметрыПроверки.ИнтерактивныйЗапуск = Истина;
	КонецЕсли;
	
	Если ПараметрыПроверки.ТаблицаИсключений <> Неопределено И ТаблицаИсключений <> Неопределено Тогда
		ИнициализироватьТаблицуИсключений();
		Для Каждого Строка Из ПараметрыПроверки.ТаблицаИсключений Цикл
			ЗаполнитьЗначенияСвойств(ТаблицаИсключений.Добавить(), Строка);
		КонецЦикла;
		
		ПараметрыПроверки.ТаблицаИсключений = ТаблицаИсключений.Скопировать();
	КонецЕсли;
	...
	ПроверятьВызовыМетодовКакПроизвольныйКод =
		СтандартныеПодсистемыСервер.ПроверятьВызовыМетодовКакПроизвольныйКод();
	
	ВыполнитьПроверкуВНесколькоПотоков(ПроверяемыеПодсистемы, ШаблонИмениПроцедуры,
		Не (ПараметрыПроверки.Свойство("ПодключеныТолькоСлужебныеРасширения")
		    И ПараметрыПроверки["ПодключеныТолькоСлужебныеРасширения"] = Истина));
...
Процедура ДобавитьОшибкуПравНаРеквизитыРоли(Знач ОбъектМетаданных, КраткоеОписаниеОшибки, ПодробноеОписаниеОшибки, 
	ПодсистемаБСП = Неопределено, Критичная = Ложь, ПраваПоУмолчанию = Неопределено)
	...
	Если ИндексТаблицы >= 0 Тогда
		...
		Пока СтрокаТаблицы.ОбъектМетаданных = ПредставлениеОбъекта И СтрокаТаблицы.ПодсистемаБСП = 
			?(ПодсистемаБСП = Неопределено, "", ПодсистемаБСП.Представление()) Цикл
...
//
Функция ПараметрыМетода(Знач ТекстМодуля, Знач ИскомыйМетод, Знач ПозицияНачалаПоиска = 1)
	
	МассивПараметров = Новый Массив;
	
	Результат = Новый Структура;
	Результат.Вставить("МетодНайден", Ложь);
	Результат.Вставить("МассивПараметров", МассивПараметров);
	Результат.Вставить("ПозицияНачала", 0);
	Результат.Вставить("ПозицияОкончания", 0);
	Результат.Вставить("ТекстВызова", "");
	Результат.Вставить("ТекстПараметров", "");
	...
	Результат.МетодНайден = Истина;
	Результат.ПозицияНачала = ПозицияВызоваМетода;
	ТекстДоМетода = Лев(ТекстМодуля, ПозицияВызоваМетода);
	Результат.НомерСтроки = СтрЧислоСтрок(ТекстДоМетода);
	...
	Если ПозицияОткрывающейсяСкобки = 0 Тогда
		Результат.ПозицияОкончания = ПозицияВызоваМетода + СтрДлина(ИскомыйМетод);
		Возврат Результат;
	КонецЕсли;
	...
	Результат.МассивПараметров = МассивПараметров;
	Результат.ПозицияОкончания = НомерСимвола;
	Результат.ТекстВызова = ТекстВызова;
	Результат.ТекстПараметров = ТекстПараметров;
...
//
// Параметры:
//   Строка                        - Строка             - исходная строка, в которой будет осуществляться поиск.
//   МассивПоиска                  - Массив из Строка   - массив подстрок, по которым осуществляется поиск.
...
//   ВернутьПервыйНайденныйЭлемент - Булево             - если Истина, то возвращает первый найденный элемент,
//                                                        иначе ближайший элемент из массива.
//
// Возвращаемое значение:
//   Структура:
//     * НайденныйЭлемент - Строка - элемент массива, который удалось найти.
...
Функция СтрНайтиПоМассиву(Строка, МассивПоиска, НаправлениеДляПоиска = Неопределено,
	...
	Результат = Новый Структура;
	Результат.Вставить("НайденныйЭлемент", "");
	Результат.Вставить("Позиция", 0);
	...
	Для Каждого ТекстПоиска Из МассивПоиска Цикл
		...
		Если (ВременнаяПозицияПоиска = 0)
			...
			Результат.НайденныйЭлемент = ТекстПоиска;
			Результат.Позиция = ТекущаяПозицияПоиска;
...
//   Структура:
//     * НайденныйЭлемент - Строка - элемент массива, который удалось найти.
//     * Успешно          - Булево - Истина, если удалось найти хотя бы один элемент.
//
...
Процедура ОчиститьПеременнуюМодульФормы(Содержимое)
	Для Каждого Элемент Из Содержимое Цикл
		Если ТипЗнч(Элемент) = Тип("Структура") Тогда
			ОчиститьПеременнуюМодульФормы(Элемент.Содержимое);
		КонецЕсли;
	КонецЦикла;
...
Процедура ПереключитьРазделение(Значение)
	Если ОбщегоНазначения.ИнформационнаяБазаФайловая() Тогда
		Константы.ИспользоватьРазделениеПоОбластямДанных.Установить(Значение);
	КонецЕсли;
КонецПроцедуры
...
#Область ПроверкаМодулейКорняКонфигурации

Функция ТаблицаДопустимыхВызововБСПВОбработчикахСобытийКорневыхМодулейКонфигурации()
	
	ТаблицаДопустимыхВызововБСП = Новый ТаблицаЗначений;
	ТаблицаДопустимыхВызововБСП.Колонки.Добавить("ИмяКорневогоМодуля");
	ТаблицаДопустимыхВызововБСП.Колонки.Добавить("ИмяОбработчикаСобытия");
	ТаблицаДопустимыхВызововБСП.Колонки.Добавить("ДопустимыйВызовПроцедурыБСП");
	
	НоваяСтрокаВызова = ТаблицаДопустимыхВызововБСП.Добавить();
	НоваяСтрокаВызова.ИмяКорневогоМодуля = "МодульУправляемогоПриложения";
	НоваяСтрокаВызова.ИмяОбработчикаСобытия = "ПередНачаломРаботыСистемы";
	НоваяСтрокаВызова.ДопустимыйВызовПроцедурыБСП = "#Если МобильныйКлиент Тогда
	...
	НоваяСтрокаВызова = ТаблицаДопустимыхВызововБСП.Добавить();
	НоваяСтрокаВызова.ИмяКорневогоМодуля = "МодульУправляемогоПриложения";
	НоваяСтрокаВызова.ИмяОбработчикаСобытия = "ПриНачалеРаботыСистемы";
	НоваяСтрокаВызова.ДопустимыйВызовПроцедурыБСП = "#Если МобильныйКлиент Тогда
	...
	НоваяСтрокаВызова = ТаблицаДопустимыхВызововБСП.Добавить();
	НоваяСтрокаВызова.ИмяКорневогоМодуля = "МодульУправляемогоПриложения";
	НоваяСтрокаВызова.ИмяОбработчикаСобытия = "ПередЗавершениемРаботыСистемы";
	НоваяСтрокаВызова.ДопустимыйВызовПроцедурыБСП = "#Если МобильныйКлиент Тогда
	...
	НоваяСтрокаВызова = ТаблицаДопустимыхВызововБСП.Добавить();
	НоваяСтрокаВызова.ИмяКорневогоМодуля = "МодульОбычногоПриложения";
	НоваяСтрокаВызова.ИмяОбработчикаСобытия = "ПередНачаломРаботыСистемы";
	НоваяСтрокаВызова.ДопустимыйВызовПроцедурыБСП = "#Если МобильныйКлиент Тогда
													|Если ОсновнойСерверДоступен() = Ложь Тогда
													|Возврат;
	...
	НоваяСтрокаВызова = ТаблицаДопустимыхВызововБСП.Добавить();
	НоваяСтрокаВызова.ИмяКорневогоМодуля = "МодульСеанса";
	НоваяСтрокаВызова.ИмяОбработчикаСобытия = "УстановкаПараметровСеанса";
	НоваяСтрокаВызова.ДопустимыйВызовПроцедурыБСП = "СтандартныеПодсистемыСервер.УстановкаПараметровСеанса(";
	
	Возврат ТаблицаДопустимыхВызововБСП;
	
КонецФункции

Процедура ПроверитьКорневыеМодулиКонфигурации()
	
	КаталогВыгрузкиКорняКонфигурации = КаталогДобавитьРазделительПути(КаталогВыгрузки) + "Ext";
	КаталогВыгрузкиКорняКонфигурации = КаталогДобавитьРазделительПути(КаталогВыгрузкиКорняКонфигурации);
	
	КорневыеМодулиКонфигурации = Новый Структура;
	КорневыеМодулиКонфигурации.Вставить("МодульУправляемогоПриложения", "ManagedApplicationModule.bsl");
	КорневыеМодулиКонфигурации.Вставить("МодульОбычногоПриложения", "OrdinaryApplicationModule.bsl");
	КорневыеМодулиКонфигурации.Вставить("МодульСеанса", "SessionModule.bsl");
	
	ТаблицаДопустимыхВызововБСП = ТаблицаДопустимыхВызововБСПВОбработчикахСобытийКорневыхМодулейКонфигурации();
	
	Для Каждого КлючИЗначение Из КорневыеМодулиКонфигурации Цикл
		
		ИмяФайла = КлючИЗначение.Значение;
		ПолноеИмяФайла = КаталогВыгрузкиКорняКонфигурации + ИмяФайла;
		
		ТекстМодуля = ПрочитатьТекстМодуля(ПолноеИмяФайла);
		Если ПустаяСтрока(ТекстМодуля) Тогда
			Продолжить;
		КонецЕсли;
		
		ИмяМодуля = КлючИЗначение.Ключ;
		
		СтруктураОтбораПоКорневомуМодулю = Новый Структура;
		СтруктураОтбораПоКорневомуМодулю.Вставить("ИмяКорневогоМодуля", ИмяМодуля);
		ТаблицаДопустимыхВызововБСППоМодулю = ТаблицаДопустимыхВызововБСП.Скопировать(СтруктураОтбораПоКорневомуМодулю);
		
		Если ИмяМодуля = "МодульУправляемогоПриложения"
		 Или ИмяМодуля = "МодульОбычногоПриложения" Тогда
			
			ПроверитьОбластьПеременныхМодуляПриложения(ТекстМодуля, ИмяМодуля);
		КонецЕсли;
		
		ПроверитьОбработчикиКорневогоМодуля(ТекстМодуля, ИмяМодуля, ТаблицаДопустимыхВызововБСППоМодулю);
		
	КонецЦикла;
	
КонецПроцедуры

Процедура ПроверитьОбластьПеременныхМодуляПриложения(ТекстМодуля, ИмяМодуля)
	
	Если ИмяМодуля = "МодульУправляемогоПриложения" Тогда
		ИмяМодуляДляВывода = НСтр("ru = 'Модуль управляемого приложения'");
	ИначеЕсли ИмяМодуля = "МодульОбычногоПриложения" Тогда
		ИмяМодуляДляВывода = НСтр("ru = 'Модуль обычного приложения'");
	Иначе
		Возврат;
	КонецЕсли;
	
	ТекстОбластьОписаниеПеременных = ВРег(КлючевоеСловоОбласть() + "ОписаниеПеременных");
	
	ТекстМодуляБезПробелов = ТекстМодуля;
	ТекстМодуляБезПробелов = СтрЗаменить(ТекстМодуляБезПробелов, Символы.Таб, "");
	ТекстМодуляБезПробелов = СтрЗаменить(ТекстМодуляБезПробелов, " ", "");
	ТекстМодуляБезПробелов = ВРег(ТекстМодуляБезПробелов);
	
	ПозицияПоиска = 0;
	Пока Истина Цикл
		
		НомерСтрокиРазделаОписанияПеременных = 0;
		ПозицияПоиска = СтрНайти(ТекстМодуляБезПробелов, ТекстОбластьОписаниеПеременных,, ПозицияПоиска + 1);
		Если ПозицияПоиска = 0 Тогда
			Прервать;
		КонецЕсли;
		
		НомерСтрокиРазделаОписанияПеременных = ПолучитьНомерСтрокиМодуля(ТекстМодуляБезПробелов, ПозицияПоиска);
		СтрокаРазделаОписанияПеременных = СтрПолучитьСтроку(ТекстМодуляБезПробелов, НомерСтрокиРазделаОписанияПеременных);
		
		Если СтрокаРазделаОписанияПеременных = ТекстОбластьОписаниеПеременных Тогда
			Прервать;
		КонецЕсли;
		
	КонецЦикла;
	
	Если НомерСтрокиРазделаОписанияПеременных = 0 Тогда
		КраткоеОписаниеОшибки = НСтр("ru = 'Область описания переменных не существует.'");
		ПодробноеОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не существует область описания переменных, в котором должна быть определена переменная %1.'"),
			"ПараметрыПриложения");
		ДобавитьОшибку(ИмяМодуляДляВывода, КраткоеОписаниеОшибки, ПодробноеОписаниеОшибки);
		Возврат;
	КонецЕсли;
	
	ТекстПерем = ВРег("Перем");
	
	ГлобальныеПеременные = Новый Массив;
	ГлобальныеПеременные.Добавить(ВРег("ПараметрыПриложения"));
	
	ПеременнаяПараметрыПриложенияОпределена = Ложь;
	
	ТекстовыйДокументТекстМодуля = Новый ТекстовыйДокумент;
	ТекстовыйДокументТекстМодуля.УстановитьТекст(ТекстМодуля);
	КоличествоСтрок = ТекстовыйДокументТекстМодуля.КоличествоСтрок();
	
	Для НомерСтроки = НомерСтрокиРазделаОписанияПеременных + 1 По КоличествоСтрок Цикл
		
		ТекущаяСтрока = ТекстовыйДокументТекстМодуля.ПолучитьСтроку(НомерСтроки);
		ТекущаяСтрока = СокрЛП(ТекущаяСтрока);
		
		Если ПустаяСтрока(ТекущаяСтрока) Или СтрНачинаетсяС(ТекущаяСтрока, "//") Тогда
			Продолжить;
		КонецЕсли;
		
		Если СтрНачинаетсяС(ТекущаяСтрока, "#КонецОбласти") Тогда
			Если НЕ ПеременнаяПараметрыПриложенияОпределена Тогда
				КраткоеОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Не определена обязательная переменная %1.'"), "ПараметрыПриложения");
				ПодробноеОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'В области описания переменных не определена обязательная переменная:
					           |
					           |%1'"),
		...
		Для Каждого ИскомаяПеременная Из ГлобальныеПеременные Цикл
			Для Каждого ЭлементМассива Из СтрРазделить(ТекущаяСтрока, ",", Ложь) Цикл
				
				Подстрока = СтрЗаменить(ВРег(ЭлементМассива), ТекстПерем, "");
				Подстрока = СокрЛП(Подстрока);
				Если СтрНачинаетсяС(Подстрока, ИскомаяПеременная) Тогда
					ПеременнаяПараметрыПриложенияОпределена = Истина;
					Продолжить;
				КонецЕсли;
				
				КраткоеОписаниеОшибки = НСтр("ru = 'Нерекомендуемые переменные в области описания переменных.'");
				ПодробноеОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Нерекомендуемые переменные в области описания переменных, стр. %1:
				               |
				               |%2
				               |
...
Процедура ПроверитьОбработчикиКорневогоМодуля(ТекстМодуля, ИмяМодуля, ТаблицаДопустимыхВызововБСП)
	
	Если ИмяМодуля = "МодульУправляемогоПриложения" Тогда
		ИмяМодуляДляВывода = НСтр("ru = 'Модуль управляемого приложения'");
		ШаблонТекстаПодробноеОписаниеОшибки =
			НСтр("ru = 'В модуле управляемого приложения не существует обязательный обработчик %1.'");
		
	ИначеЕсли ИмяМодуля = "МодульОбычногоПриложения" Тогда
		ИмяМодуляДляВывода = НСтр("ru = 'Модуль обычного приложения'");
		ШаблонТекстаПодробноеОписаниеОшибки =
			НСтр("ru = 'В модуле обычного приложения не существует обязательный обработчик %1.'");
		
	ИначеЕсли ИмяМодуля = "МодульСеанса" Тогда
		ИмяМодуляДляВывода = НСтр("ru = 'Модуль сеанса'");
		ШаблонТекстаПодробноеОписаниеОшибки =
			НСтр("ru = 'В модуле сеанса не существует обязательный обработчик %1.'");
	Иначе
		Возврат;
	КонецЕсли;
	
	ТекстПроцедура = "Процедура";
	ТекстКонецПроцедуры = "КонецПроцедуры";
	
	ТаблицаВызовов = СоздатьТаблицуВызовов();
	
	ШаблонТекстаКраткоеОписаниеОшибки = НСтр("ru = 'Не существует обязательный обработчик %1.'");
	
	ТаблицаПроверяемыхОбработчиков = ТаблицаДопустимыхВызововБСП.Скопировать();
	ТаблицаПроверяемыхОбработчиков.Свернуть("ИмяОбработчикаСобытия");
	
	ТекстМодуляБуфер = ВРег(ТекстМодуля);
	ТекстМодуляБуфер = СтрЗаменить(ТекстМодуляБуфер, Символы.Таб, "");
	ТекстМодуляБуфер = СтрЗаменить(ТекстМодуляБуфер, " ", "");
	Для Каждого СтрокаПроверяемогоОбработчика Из ТаблицаПроверяемыхОбработчиков Цикл
		
		ПроверяемыйОбработчик = СтрокаПроверяемогоОбработчика.ИмяОбработчикаСобытия;
		КонструкцияПоиска = ВРег(ТекстПроцедура + ПроверяемыйОбработчик + "(");
		
		ПозицияНачалаОбработчика = 0;
		Пока Истина Цикл
			
			ПозицияНачалаОбработчика = СтрНайти(ТекстМодуляБуфер, КонструкцияПоиска,, ПозицияНачалаОбработчика + 1);
			Если ПозицияНачалаОбработчика = 0 Тогда
				Прервать;
			КонецЕсли;
			
			ПредыдущийСимвол = Сред(ТекстМодуляБуфер, ПозицияНачалаОбработчика - 1, 1);
		...
		Если ПозицияНачалаОбработчика = 0 Тогда
			ПодробноеОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ШаблонТекстаПодробноеОписаниеОшибки, ПроверяемыйОбработчик);
			КраткоеОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ШаблонТекстаКраткоеОписаниеОшибки, ПроверяемыйОбработчик);
			ДобавитьОшибку(ИмяМодуляДляВывода, КраткоеОписаниеОшибки, ПодробноеОписаниеОшибки);
			Продолжить;
		КонецЕсли;
		
		ПозицияКонцаОбработчика = 0;
		Пока Истина Цикл
			
			ПозицияКонцаОбработчика = СтрНайти(ТекстМодуляБуфер, ВРег(ТекстКонецПроцедуры),, ПозицияНачалаОбработчика);
			Если ПозицияКонцаОбработчика = 0 Тогда
				Прервать;
			КонецЕсли;
			
			ПредыдущийСимвол = Сред(ТекстМодуляБуфер, ПозицияКонцаОбработчика - 1, 1);
			Если ПустаяСтрока(ПредыдущийСимвол) Тогда
				Прервать;
			КонецЕсли;
		...
		НомерСтрокиНачалаОбработчика = ПолучитьНомерСтрокиМодуля(ТекстМодуляБуфер, ПозицияНачалаОбработчика);
		НомерСтрокиКонцаОбработчика = ПолучитьНомерСтрокиМодуля(ТекстМодуляБуфер, ПозицияКонцаОбработчика);
		
		ТекстОбработчика = "";
		Для НомерСтроки = НомерСтрокиНачалаОбработчика + 1 По НомерСтрокиКонцаОбработчика - 1 Цикл
			
			СтрокаОбработчика = СокрЛП(СтрПолучитьСтроку(ТекстМодуля, НомерСтроки));
			Если ПустаяСтрока(СтрокаОбработчика)
				ИЛИ СтрНачинаетсяС(СтрокаОбработчика, "//") Тогда
				Продолжить;
			КонецЕсли;
			
			ТекстОбработчика = ТекстОбработчика + СтрокаОбработчика + Символы.ПС;
			
		КонецЦикла;
		
		ТекстОбработчика = СокрЛП(ТекстОбработчика);
		
		СтруктураОтбораПоОбработчику = Новый Структура;
		СтруктураОтбораПоОбработчику.Вставить("ИмяОбработчикаСобытия", ПроверяемыйОбработчик);
		ТаблицаДопустимыхВызововПоОбработчику = ТаблицаДопустимыхВызововБСП.Скопировать(СтруктураОтбораПоОбработчику);
		Для Каждого СтрокаДопустимогоВызова Из ТаблицаДопустимыхВызововПоОбработчику Цикл
			
			ДопустимыйВызовПроцедурыБСП = СтрокаДопустимогоВызова.ДопустимыйВызовПроцедурыБСП;
			
			ПозицияВызова = СтрНайти(ТекстОбработчика, ДопустимыйВызовПроцедурыБСП);
			Если ПозицияВызова = 0 Тогда
				КраткоеОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					ШаблонТекстаКраткоеОписаниеОшибки, ПроверяемыйОбработчик);
				ПодробноеОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			// Удаляем строки обработчика.
			НомерСтрокиВызова = ПолучитьНомерСтрокиМодуля(ТекстОбработчика, ПозицияВызова);
			КоличествоСтрокВызова = СтрЧислоСтрок(ДопустимыйВызовПроцедурыБСП);
			
			// Удаляем строки с допустимым вызовом.
			ДокументТекстОбработчика = Новый ТекстовыйДокумент;
			ДокументТекстОбработчика.УстановитьТекст(ТекстОбработчика);
			Для НомерСтрокиДопустимогоВызова = 1 По КоличествоСтрокВызова Цикл
				ДокументТекстОбработчика.ЗаменитьСтроку(НомерСтрокиВызова + НомерСтрокиДопустимогоВызова - 1, "");
			КонецЦикла;
			
			ТекстОбработчика = ДокументТекстОбработчика.ПолучитьТекст();
		...
		ТекстОбработчика = СокрЛП(ТекстОбработчика);
		КоличествоСтрокОбработчика = СтрЧислоСтрок(ТекстОбработчика);
		
		ПерваяСтрокаНедопустимогоВызова = "";
		Для НомерСтрокиОбработчика = 1 По КоличествоСтрокОбработчика Цикл
			
			ПерваяСтрокаНедопустимогоВызова = СтрПолучитьСтроку(ТекстОбработчика, НомерСтрокиОбработчика);
			ПерваяСтрокаНедопустимогоВызова = СокрЛП(ПерваяСтрокаНедопустимогоВызова);
			
			Если ПустаяСтрока(ПерваяСтрокаНедопустимогоВызова) Тогда
				Продолжить;
			КонецЕсли;
			
			Если СтрНачинаетсяС(ПерваяСтрокаНедопустимогоВызова, "#") Тогда
				Продолжить;
			КонецЕсли;
		...
		ПозицияНедопустимогоВызова = СтрНайти(ТекстМодуля, ПерваяСтрокаНедопустимогоВызова,, ПозицияНачалаОбработчика);
		Если ПозицияНедопустимогоВызова > 0 Тогда
			НомерСтрокиНедопустимогоВызова = ПолучитьНомерСтрокиМодуля(ТекстМодуля, ПозицияНедопустимогоВызова);
		КонецЕсли;
		
		НовыйВызов = ТаблицаВызовов.Добавить();
		НовыйВызов.Строка = ПерваяСтрокаНедопустимогоВызова;
		НовыйВызов.НомерСтроки = НомерСтрокиНедопустимогоВызова;
		НовыйВызов.ДополнительнаяИнформация = СтрокаПроверяемогоОбработчика.ИмяОбработчикаСобытия;
		
	КонецЦикла;
	
	Если ИмяМодуля = "МодульУправляемогоПриложения" Тогда
		ШаблонПодробногоОписанияОшибки =
			НСтр("ru = 'Недопустимый вызов в обработчике %1 модуля управляемого приложения, стр. %2:'");
	ИначеЕсли ИмяМодуля = "МодульОбычногоПриложения" Тогда
		ШаблонПодробногоОписанияОшибки =
			НСтр("ru = 'Недопустимый вызов в обработчике %1 модуля обычного приложения, стр. %2:'");
	ИначеЕсли ИмяМодуля = "МодульСеанса" Тогда
		ШаблонПодробногоОписанияОшибки =
			НСтр("ru = 'Недопустимый вызов в обработчике %1 модуля сеанса, стр. %2:'");
	КонецЕсли;
	
	ШаблонПодробногоОписанияОшибки = ШаблонПодробногоОписанияОшибки + Символы.ПС + Символы.ПС
		+ НСтр("ru = '%3
		             |
	...
	УточнениеМодуляПриложения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'А прикладной код должен быть размещен в одноименном обработчике в общем модуле %1.'"),
		"ОбщегоНазначенияКлиентПереопределяемый");
	УточнениеМодуляСеанса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'А прикладной код должен быть размещен в своем общем модуле и процедуре, ссылка на которую должна быть добавлена в процедуре %1 общего модуля %2.'"),
		"ПриДобавленииОбработчиковУстановкиПараметровСеанса",
		"ОбщегоНазначенияПереопределяемый");
	
	Для Каждого Вызов Из ТаблицаВызовов Цикл
		КраткоеОписаниеОшибки =
			НСтр("ru = 'Некорректное размещение кода обработки события модуля приложения.'");
		ПодробноеОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонПодробногоОписанияОшибки,
			Вызов.ДополнительнаяИнформация, Вызов.НомерСтроки, Вызов.Строка);
		Если Вызов.ДополнительнаяИнформация = "УстановкаПараметровСеанса" Тогда
			ПодробноеОписаниеОшибки = ПодробноеОписаниеОшибки + " " + УточнениеМодуляСеанса;
		Иначе
			ПодробноеОписаниеОшибки = ПодробноеОписаниеОшибки + " " + УточнениеМодуляПриложения;
		КонецЕсли;
		ДобавитьОшибку(ИмяМодуляДляВывода, КраткоеОписаниеОшибки, ПодробноеОписаниеОшибки);
	КонецЦикла;
...
Функция ПоследняяСтрокаМодуля(Знач ТекстМодуля)
...
Функция КаталогДобавитьРазделительПути(Знач Каталог)
...
Функция СоздатьТаблицуВызовов()
	
	ТаблицаВызовов = Новый ТаблицаЗначений;
	ТаблицаВызовов.Колонки.Добавить("НомерСтроки");
	ТаблицаВызовов.Колонки.Добавить("Строка");
	ТаблицаВызовов.Колонки.Добавить("ДополнительнаяИнформация");
...
#Область ПроверкаВНесколькоПотоков

// Запускает проверку в многопоточном режиме с обработкой результата.
//
// Параметры:
//   ИмяМетода                       - Строка            - имя процедуры запускаемой проверки.
//   ПодсистемаБСП                   - Строка            - имя проверяемой подсистемы.
//   ХранилищеСтруктурыИнициализации - ХранилищеЗначения - хранилище, содержащее значения переменных модуля
//                                                         объекта отчета.
//
// Возвращаемое значение:
//   ТаблицаЗначений - табличная часть отчета ТаблицаПроверки, выгруженная в таблицу значений.
//
// АПК:581-выкл Экспортная, так как вызывается из фонового задания.
// АПК:299-выкл Используется для запуска в многопоточном режиме.
// АПК:487-выкл безопасный вызов.
//
Функция РезультатПроверкиПодсистемы(ИмяМетода, ПодсистемаБСП, ХранилищеСтруктурыИнициализации, Проверка = Неопределено) Экспорт
	...
	ИнициализацияПеременныхМодуляОбъектаОтчета(ХранилищеСтруктурыИнициализации, ПодсистемаБСП);
	
	ПроверяемыеОбъекты = УправлениеДоступом_ПроверяемыеОбъектыМетаданных(Проверка);
...
КонецФункции
// АПК:299-вкл, АПК:581-вкл, АПК:487-вкл

Процедура ВыполнитьПроверкуВНесколькоПотоков(ПроверяемыеПодсистемы, ШаблонИмениПроцедуры, БезРасширений)
	...
	ПараметрыВыполнения.БезРасширений = БезРасширений;
	ПараметрыВыполнения.ОжидатьЗавершение = Неопределено; // Ждать завершения или вызвать исключение.
	
	ЭтоВнешнийОтчет = ЭтоВнешнийОтчет(ДвоичныеДанныеВнешнегоОтчета);
	
	Если ЭтоВнешнийОтчет Тогда
		Если Не ЗначениеЗаполнено(ПолноеИмяФайлаВнешнегоОтчета) Тогда
			ПараметрыВыполнения.ВнешнийОтчетОбработка = ДвоичныеДанныеВнешнегоОтчета;
		Иначе
			ПараметрыВыполнения.ВнешнийОтчетОбработка = Новый Структура("ДвоичныеДанные, ПолноеИмяФайла",
				ДвоичныеДанныеВнешнегоОтчета, ПолноеИмяФайлаВнешнегоОтчета);
		КонецЕсли;
		Если ДвоичныеДанныеВнешнегоОтчета = Неопределено
	...
	ХранилищеСтруктурыИнициализации = Новый ХранилищеЗначения(СтруктураЗначенийПеременныхМодуляОбъектаОтчета());
	
	Для Каждого Подсистема Из ПроверяемыеПодсистемы Цикл
		ИмяПодсистемы = ?(ТипЗнч(ПроверяемыеПодсистемы) = Тип("Массив"), Подсистема, Подсистема.Значение);
		
		ПроверяемаяПодсистема = Метаданные.Подсистемы.СтандартныеПодсистемы.Подсистемы.Найти(ИмяПодсистемы);
		Если ПроверяемаяПодсистема <> Неопределено Тогда
			ДобавитьПараметрМетодаМногопоточнойОперации(ПараметрыМетода, ШаблонИмениПроцедуры, ИмяПодсистемы,
				ХранилищеСтруктурыИнициализации);
		КонецЕсли;
	КонецЦикла;
	
	Если ЭтоВнешнийОтчет Тогда
		РезультатВыполнения = ДлительныеОперации.ВыполнитьФункциюВНесколькоПотоков(
			"ВнешнийОтчет.ПроверкаВнедренияБСП.МодульОбъекта.РезультатПроверкиПодсистемы",
			ПараметрыВыполнения,
			ПараметрыМетода);
	Иначе
		РезультатВыполнения = ДлительныеОперации.ВыполнитьФункциюВНесколькоПотоков(
			"Отчет.ПроверкаВнедренияБСП.МодульОбъекта.РезультатПроверкиПодсистемы",
			ПараметрыВыполнения,
			ПараметрыМетода);
	КонецЕсли;
	...
	Если РезультатВыполнения.Статус <> "Выполнено" Тогда
		...
		ИначеЕсли РезультатВыполнения.Статус = "Отменено" Тогда
			КраткоеПредставлениеОшибки = НСтр("ru = 'Проверка отменена.'");
			ПодробноеПредставлениеОшибки = КраткоеПредставлениеОшибки;
		Иначе
			КраткоеПредставлениеОшибки = НСтр("ru = 'Проверка завершилась неуспешно.'");
			ПодробноеПредставлениеОшибки = КраткоеПредставлениеОшибки;
		КонецЕсли;
		ДобавитьОшибку(Неопределено, КраткоеПредставлениеОшибки,
	...
	Если ТипЗнч(АдресаРезультатовПроверок) <> Тип("Соответствие") Тогда
		ТекстОшибки = НСтр("ru = 'Нет результатов проверки.'");
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
	
	Для Каждого АдресРезультатаПроверки Из АдресаРезультатовПроверок Цикл
		
		РезультатПроверки = АдресРезультатаПроверки.Значение;
		Если РезультатПроверки.Статус <> "Выполнено" Тогда
			Если РезультатПроверки.Статус = "Ошибка" Тогда
				ТекстОшибки = РезультатПроверки.ПодробноеПредставлениеОшибки;
			ИначеЕсли РезультатПроверки.Статус = "Отменено" Тогда
				ТекстОшибки = НСтр("ru = 'Проверка отменена.'");
			Иначе
				ТекстОшибки = НСтр("ru = 'Проверка завершилась неуспешно.'");
			КонецЕсли;
			ДобавитьОшибку(Неопределено, АдресРезультатаПроверки.Ключ,
				ТекстОшибки,, Истина);
			Продолжить;
		Иначе
			ТаблицаРезультатаПроверки = ПолучитьИзВременногоХранилища(РезультатПроверки.АдресРезультата);
			УдалитьИзВременногоХранилища(РезультатПроверки.АдресРезультата);
			Если ТипЗнч(ТаблицаРезультатаПроверки) <> Тип("Структура") Тогда
				ДобавитьОшибку(Неопределено, АдресРезультатаПроверки.Ключ,
					НСтр("ru = 'Нет результата проверки.'"),, Истина);
				Продолжить;
			КонецЕсли;
		КонецЕсли;
		
		Для Каждого СтрокаТаблицыРезультатаПроверки Из ТаблицаРезультатаПроверки.ТаблицаПроверки Цикл
			ЗаполнитьЗначенияСвойств(ТаблицаПроверки.Добавить(), СтрокаТаблицыРезультатаПроверки);
		КонецЦикла;
		
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ЗагружаемыеФайлы, ТаблицаРезультатаПроверки.ЗагружаемыеФайлы);
		
	КонецЦикла;
	
	ТаблицаПроверки.Сортировать("ОбъектМетаданных Возр, ПодсистемаБСП Возр");
	
КонецПроцедуры

Процедура ДобавитьПараметрМетодаМногопоточнойОперации(ПараметрыМетода, ШаблонИмениПроцедуры, ИмяПодсистемы,
		ХранилищеСтруктурыИнициализации)
	
	ИмяПроцедуры = СтрЗаменить(ШаблонИмениПроцедуры, "[ИмяПодсистемы]", ИмяПодсистемы);
	МассивПараметров = Новый Массив;
	МассивПараметров.Добавить(ИмяПроцедуры);
	МассивПараметров.Добавить(ИмяПодсистемы);
	МассивПараметров.Добавить(ХранилищеСтруктурыИнициализации);
	
	ПараметрыМетода.Вставить(ИмяПроцедуры, МассивПараметров);
	
	Если СтрСравнить(ИмяПодсистемы, "УправлениеДоступом") <> 0 Тогда
		Возврат;
	КонецЕсли;
	
	ПроверкиПодсистемыУправлениеДоступом = УправлениеДоступом_МассивПроверок();
	УправлениеДоступом_ШаблонИмениПроцедуры = "%1(%2)";
	
	Для Каждого СведенияОПроверке Из ПроверкиПодсистемыУправлениеДоступом Цикл
		ИмяПроцедуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(УправлениеДоступом_ШаблонИмениПроцедуры, 
			СведенияОПроверке.Значение, "ПроверяемыеОбъекты");
		
		МассивПараметров = Новый Массив;
		МассивПараметров.Добавить(ИмяПроцедуры);
		МассивПараметров.Добавить(ИмяПодсистемы);
		МассивПараметров.Добавить(ХранилищеСтруктурыИнициализации);
		МассивПараметров.Добавить(СведенияОПроверке.Ключ);
...
Функция СтруктураЗначенийПеременныхМодуляОбъектаОтчета()
	
	СтруктураРезультат = Новый Структура;
	СтруктураРезультат.Вставить("ТаблицаИсключений",                           ТаблицаИсключений);
	СтруктураРезультат.Вставить("КаталогВыгрузки",                             КаталогВыгрузки);
	СтруктураРезультат.Вставить("ОтборПоПодсистемам",                          ОтборПоПодсистемам);
	СтруктураРезультат.Вставить("ДопустимыеМетаданные",                        ДопустимыеМетаданные);
	СтруктураРезультат.Вставить("ИсправлятьОшибки",                            ИсправлятьОшибки);
	СтруктураРезультат.Вставить("ЗагружаемыеФайлы",                            ЗагружаемыеФайлы);
	СтруктураРезультат.Вставить("ИсправляемыеОшибки",                          ИсправляемыеОшибки);
	СтруктураРезультат.Вставить("НеПроверяемыеПодсистемыКонфигурации",         НеПроверяемыеПодсистемыКонфигурации);
	СтруктураРезультат.Вставить("ИнтерактивныйЗапуск",                         ИнтерактивныйЗапуск);
	СтруктураРезультат.Вставить("ИмяВидаОбъектаМетаданныхНаЯзыкеКонфигурации", ИмяВидаОбъектаМетаданныхНаЯзыкеКонфигурации);
	СтруктураРезультат.Вставить("ИмяВидаОбъектаВВыгрузкеФайлов",               ИмяВидаОбъектаВВыгрузкеФайлов);
	СтруктураРезультат.Вставить("ЕстьОшибкиСоставаРазделителей",               ЕстьОшибкиСоставаРазделителей);
	СтруктураРезультат.Вставить("КонфигурацияEDT",                             КонфигурацияEDT);
	СтруктураРезультат.Вставить("ПроверятьВызовыМетодовКакПроизвольныйКод",    ПроверятьВызовыМетодовКакПроизвольныйКод);
	
	Возврат СтруктураРезультат;
	
КонецФункции

Процедура ИнициализацияПеременныхМодуляОбъектаОтчета(ХранилищеСтруктурыИнициализации, ПодсистемаБСП)
	
	ПроверяемаяПодсистема = Метаданные.Подсистемы.СтандартныеПодсистемы.Подсистемы.Найти(ПодсистемаБСП);
	Если ПроверяемаяПодсистема = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Если ХранилищеСтруктурыИнициализации = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	СтруктураИнициализации = ХранилищеСтруктурыИнициализации.Получить();
	
	ТаблицаИсключений                           = СтруктураИнициализации.ТаблицаИсключений;
	КаталогВыгрузки                             = СтруктураИнициализации.КаталогВыгрузки;
	ОтборПоПодсистемам                          = СтруктураИнициализации.ОтборПоПодсистемам;
	ДопустимыеМетаданные                        = СтруктураИнициализации.ДопустимыеМетаданные;
	ИсправлятьОшибки                            = СтруктураИнициализации.ИсправлятьОшибки;
	ЗагружаемыеФайлы                            = СтруктураИнициализации.ЗагружаемыеФайлы;
	ИсправляемыеОшибки                          = СтруктураИнициализации.ИсправляемыеОшибки;
	НеПроверяемыеПодсистемыКонфигурации         = СтруктураИнициализации.НеПроверяемыеПодсистемыКонфигурации;
	ИнтерактивныйЗапуск                         = СтруктураИнициализации.ИнтерактивныйЗапуск;
	ИмяВидаОбъектаМетаданныхНаЯзыкеКонфигурации = СтруктураИнициализации.ИмяВидаОбъектаМетаданныхНаЯзыкеКонфигурации;
	ИмяВидаОбъектаВВыгрузкеФайлов               = СтруктураИнициализации.ИмяВидаОбъектаВВыгрузкеФайлов;
	ЕстьОшибкиСоставаРазделителей               = СтруктураИнициализации.ЕстьОшибкиСоставаРазделителей;
	КонфигурацияEDT                             = СтруктураИнициализации.КонфигурацияEDT;
	ПроверятьВызовыМетодовКакПроизвольныйКод    = СтруктураИнициализации.ПроверятьВызовыМетодовКакПроизвольныйКод;
...
Функция ЭтоВнешнийОтчет(ДвоичныеДанныеВнешнегоОтчета)
	
	СтруктураОбъекта = Новый Структура;
	СтруктураОбъекта.Вставить("ИспользуемоеИмяФайла", Неопределено);
	ЗаполнитьЗначенияСвойств(СтруктураОбъекта, ЭтотОбъект);
	
	Если СтруктураОбъекта.ИспользуемоеИмяФайла = Неопределено Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Если ДвоичныеДанныеВнешнегоОтчета <> Неопределено Тогда
		Возврат Истина;
	КонецЕсли;
	
	Файл = Новый Файл(СтруктураОбъекта.ИспользуемоеИмяФайла);
	Если Файл.Существует() Тогда
		ДвоичныеДанныеВнешнегоОтчета = Новый ДвоичныеДанные(СтруктураОбъекта.ИспользуемоеИмяФайла);
		ПолноеИмяФайлаВнешнегоОтчета = СтруктураОбъекта.ИспользуемоеИмяФайла;
	КонецЕсли;
...
#Область УправлениеДоступом_РазделениеНаНесколькоПотоков

Процедура Подключаемый_УправлениеДоступом_ПроверитьВставкуТекстаВМодулеФормы(КоллекцияОбъектовМетаданных)
	
	Если Не ТребуетсяВыполнениеПроверки() Тогда
		Возврат;
	КонецЕсли;
...
Процедура Подключаемый_УправлениеДоступом_ПроверитьИспользованиеПредопределенныхВМодулеФормы(КоллекцияОбъектовМетаданных)
...
Процедура Подключаемый_УправлениеДоступом_ПроверитьИспользованиеПредопределенныхВОбщемМодуле(КоллекцияОбъектовМетаданных)
...
Процедура Подключаемый_УправлениеДоступом_ПроверитьИспользованиеПредопределенныхВМодуляхОбъекта(КоллекцияОбъектовМетаданных)
...
Процедура Подключаемый_УправлениеДоступом_ПроверитьИспользованиеПредопределенныхВМодулеКоманды(КоллекцияОбъектовМетаданных)
...
Процедура Подключаемый_УправлениеДоступом_ПроверитьВставкуПроцедурыПриЗаполненииОграниченияДоступаВМодулеМенеджераОбъекта(КоллекцияОбъектовМетаданных)
...
Функция УправлениеДоступом_МассивПроверок()
	
	Проверки = Новый Структура;
	Проверки.Вставить("ПроверитьВставкуТекстаВМодулеФормы", "Подключаемый_УправлениеДоступом_ПроверитьВставкуТекстаВМодулеФормы");
	Проверки.Вставить("ПроверитьИспользованиеПредопределенныхВМодулеФормы", "Подключаемый_УправлениеДоступом_ПроверитьИспользованиеПредопределенныхВМодулеФормы");
	Проверки.Вставить("ПроверитьИспользованиеПредопределенныхВОбщемМодуле", "Подключаемый_УправлениеДоступом_ПроверитьИспользованиеПредопределенныхВОбщемМодуле");
	Проверки.Вставить("ПроверитьИспользованиеПредопределенныхВМодуляхОбъекта", "Подключаемый_УправлениеДоступом_ПроверитьИспользованиеПредопределенныхВМодуляхОбъекта");
	Проверки.Вставить("ПроверитьИспользованиеПредопределенныхВМодулеКоманды", "Подключаемый_УправлениеДоступом_ПроверитьИспользованиеПредопределенныхВМодулеКоманды");
	Проверки.Вставить("ПроверитьВставкуПроцедурыПриЗаполненииОграниченияДоступаВМодулеМенеджераОбъекта", "Подключаемый_УправлениеДоступом_ПроверитьВставкуПроцедурыПриЗаполненииОграниченияДоступаВМодулеМенеджераОбъекта");
...
Функция УправлениеДоступом_ПроверяемыеОбъектыМетаданных(Проверка)
	...
	КоллекцииОбъектовМетаданных = КоллекцииОбъектовМетаданных();
	
	// Формируем свой массив имен объектов для каждой проверки.
	Для Каждого КоллекцияОбъектовМетаданных Из КоллекцииОбъектовМетаданных Цикл
		Для Каждого ОбъектМетаданных Из КоллекцияОбъектовМетаданных Цикл
			
			Если Метаданные.ОбщиеФормы.Содержит(ОбъектМетаданных) Тогда
				Если Проверка = "ПроверитьВставкуТекстаВМодулеФормы"
				 Или Проверка = "ПроверитьИспользованиеПредопределенныхВМодулеФормы" Тогда
					Результат.Добавить(ОбъектМетаданных);
				КонецЕсли;
				
			ИначеЕсли Метаданные.ОбщиеКоманды.Содержит(ОбъектМетаданных) Тогда
				Если Проверка = "ПроверитьИспользованиеПредопределенныхВМодулеКоманды" Тогда
					Результат.Добавить(ОбъектМетаданных);
				КонецЕсли;
				
			ИначеЕсли Метаданные.ОбщиеМодули.Содержит(ОбъектМетаданных) Тогда
				Если Проверка = "ПроверитьИспользованиеПредопределенныхВОбщемМодуле" Тогда
					Результат.Добавить(ОбъектМетаданных);
				КонецЕсли;
			Иначе
				Если Проверка = "ПроверитьВставкуПроцедурыПриЗаполненииОграниченияДоступаВМодулеМенеджераОбъекта"
				 Или Проверка = "ПроверитьИспользованиеПредопределенныхВМодуляхОбъекта" Тогда
					Результат.Добавить(ОбъектМетаданных);
				КонецЕсли;
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("РезультатПроверкиПодсистемы", Истина);
