...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	// Имитация ошибки. Проверка корректной отработки сценария, когда некорректно заполнено свойство ПараметрыВыборки.
	// Начало имитации ошибки.
	ИмитироватьОшибку = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить("ОбновлениеИБ", "ИмитироватьОшибкуВПараметрахВыборки", Ложь);
	Если ИмитироватьОшибку Тогда
		ПараметрыВыборки.СпособВыборки = "НекорректныйСпособВыборки";
		ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("ОбновлениеИБ", "ИмитироватьОшибкуВПараметрахВыборки", Ложь);
	КонецЕсли;
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Если ИмитироватьОшибку Тогда
		...
		Если Параметры.КоличествоЗапусков = 16 Тогда
			ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("ОбновлениеИБ", "ИмитироватьОшибкуПриОтложенномПараллельномОбновлении", Ложь);
		КонецЕсли;
		
		// Если есть данные для обработки, то имитация зацикливания выполняется через отмену транзакции после записи данных.
		Если ОбновлениеИнформационнойБазы.ОбработкаДанныхЗавершена(Параметры.Очередь, "Документ._ДемоЗаказПокупателя") Тогда
			Возврат;
		КонецЕсли;
	КонецЕсли;
	...
	Если ИмитироватьПроблемыСДаннымиИОбработчиком Тогда
		ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("ОбновлениеИБ", "ИмитироватьПроблемыСДаннымиИОбработчиком", Ложь);
	КонецЕсли;
	...
	Для каждого ЗаказПокупателя Из ЗаказыПокупателя Цикл
		ПредставлениеСсылки = Строка(ЗаказПокупателя.Ссылка);
		Попытка
			
			ЗаполнитьСтатусЗаказаПокупателя(ЗаказПокупателя, ИмитироватьОшибку);
			ОбъектовОбработано = ОбъектовОбработано + 1;
			
			// Начало имитации ошибки.
			Если ИмитироватьПроблемыСДаннымиИОбработчиком
				И Не Параметры.Свойство("ТестовыеОшибкиДобавлены") Тогда
				Комментарий = НСтр("ru = 'Тестовая ошибка для проверки работы отложенного обновления данных.'");
...
//
Процедура ЗаполнитьСтатусЗаказаПокупателя(ЗаказПокупателя, ИмитироватьОшибку)
	...
	Попытка
		...
		// Если включена имитация ошибки, то зацикливание имитируется при помощи отката транзакции.
		Если ИмитироватьОшибку Тогда
			ОтменитьТранзакцию();
		Иначе
			ЗафиксироватьТранзакцию();
