...
#Область ИнтерфейсДляВызоваИзДополнительнойПечатнойФормы
...
Функция ПечатьСчетаЗаказа(МассивОбъектов, ОбъектыПечати, ИмяМакета = "Счет", ВыводитьПлатежныеРеквизиты = Истина, КодЯзыка = Неопределено) Экспорт
	...
	|		Номенклатура КАК Номенклатура) КАК Товары,
	|	ВЫБОР
	...
	|	_ДемоОрганизации.НаименованиеСокращенное КАК НаименованиеПоставщика
	|ИЗ
	...
	Пока Шапка.Следующий() Цикл
		...
		ДанныеПечати.Вставить("ПредставлениеПоставщика", Шапка.НаименованиеПоставщика + " " + КонтактнаяИнформация.ЮридическийАдрес);
		...
		ДанныеПечати.Вставить("ФИОРуководителя", Шапка.Директор);
		ДанныеПечати.Вставить("ФИОБухгалтера", Шапка.ГлавныйБухгалтер);
...
Функция ПечатьКвитанции(МассивОбъектов, ОбъектыПечати) Экспорт
	...
	ТабличныйДокумент = Новый ТабличныйДокумент;
	МакетПечатнойФормы = УправлениеПечатью.МакетПечатнойФормы("Документ._ДемоСчетНаОплатуПокупателю.ПФ_MXL_Квитанция");
	...
	Для Каждого РеквизитыПлатежа Из ДанныеДокументов Цикл
		...
		ПервыйДокумент = Ложь;
		ОбластьПечатается = МакетПечатнойФормы.ПолучитьОбласть("Печатается");
		...
		УправлениеПечатью.ЗадатьОбластьПечатиДокумента(ТабличныйДокумент, НомерПервойСтроки, ОбъектыПечати, 
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Если Пользователи.ЭтоСеансВнешнегоПользователя() Тогда
		ОбъектАвторизации = ВнешниеПользователи.ПолучитьОбъектАвторизацииВнешнегоПользователя();
...
Процедура ПриОпределенииПолучателей(ПараметрыОтправки, МассивОбъектов, КоллекцияПечатныхФорм) Экспорт
	...
	Если СведенияОПокупателе <> Неопределено Тогда
		...
		АдресаПартнера = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъектов(ОбъектыКонтактнойИнформации, 
			ТипыКонтактнойИнформации,, ТекущаяДатаСеанса());
...
#Область ПодготовкаТабличныхПечатныхДокументов
